...
The editor shell and deterministic 2D/quantity projections load independently of the Three.js runtime. The 3D view, guided house-connection editor, level/cutout controls, platform commands, shared-landing connection controls, and level-alignment command are lazy browser chunks with visible status fallbacks. A post-build check enforces gzip budgets of 99 KiB for the initial entry, 170 KiB for the largest JavaScript chunk, and 242 KiB for all JavaScript combined. Exact level-side connection with deterministic destination-layer alignment measures 97.6 KiB initial and 241.7 KiB total. The total ceiling moved by one KiB while the initial entry remains below the prior level-layer slice, retaining a narrow regression gate for the added validation and alignment path. Local economy, balanced, and detailed tiers bound pixel ratio and shadow cost without entering the authoritative design document or changing projections. These are prototype regression limits, not production service-level guarantees; later device testing can tighten them.
