...
The editor shell and deterministic 2D/quantity projections load independently of the Three.js runtime. The 3D view, guided house-connection editor, level/cutout controls, platform commands, and shared-landing connection controls are lazy browser chunks with visible status fallbacks. A post-build check enforces gzip budgets of 98 KiB for the initial entry, 170 KiB for the largest JavaScript chunk, and 238 KiB for all JavaScript combined. The multi-level/cutout slice measured 97.1 KiB initial and 237.2 KiB total. The ceilings were raised only enough to retain a measurable regression gate for combined level context, exact cutout editing, and validation. Local economy, balanced, and detailed tiers bound pixel ratio and shadow cost without entering the authoritative design document or changing projections. These are prototype regression limits, not production service-level guarantees; later device testing can tighten them.
