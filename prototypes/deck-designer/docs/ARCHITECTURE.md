...
Before a traced polygon becomes DeckDesign v3, the dialog owns a bounded local undo stack for the outline and temporary stair placement. Offset creation, each completed pointer drag with a real validated change, each focused exact-dimension edit, and rectangle reset record one prior temporary snapshot. A tap, sub-snap gesture, move back to origin, cancel, lost capture, non-owning pointer, or second-pointer pinch takeover restores or retains the starting facts without adding Undo. Undo restores only this ephemeral trace state; it does not write history into the design document or interfere with the main editor's monotonic command history.
