...
The isolated v5 foundation adds sparse `edgeFinishes[]` facts to each platform. Every entry references one exact current free outer edge and explicitly records fascia and/or skirting intent; house-attached edges and empty or duplicate entries are rejected. V1–v4 migration defaults to an empty list so no finish intent is invented. Finish geometry uses the same authoritative stair openings to split selected edges, producing stable fascia-span and skirting-panel IDs. Generic visualization projections report fascia linear feet and skirting area from recorded geometry and grade only, without product, ventilation, access, waste, labor, price, or code conclusions. Safe polygon replacement preserves or uniquely remaps these exact references and fails with `review_required` for split, ambiguous, or missing selected edges. V5 is not yet the browser authority; a separate Finishes stage waits behind migration, round-trip, equivalence, history, storage, 2D/3D, quantity, and touch/browser gates.
