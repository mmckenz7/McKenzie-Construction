...
The editor shell and deterministic 2D/quantity projections load independently of the Three.js runtime. The 3D view and guided house-connection editor are lazy browser chunks with visible status fallbacks. A post-build check enforces gzip budgets of 95 KiB for the initial entry, 170 KiB for the largest JavaScript chunk, and 232 KiB for all JavaScript combined. The grouped stair/landing slice measured 94.5 KiB initial and 231.5 KiB total; the ceilings were raised only enough to retain a measurable regression gate for the added authoritative model, route engine, controls, and validation. Local economy, balanced, and detailed tiers bound pixel ratio and shadow cost without entering the authoritative design document or changing projections. These are prototype regression limits, not production service-level guarantees; later device testing can tighten them.
