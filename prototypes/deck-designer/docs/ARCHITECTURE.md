...
Every free platform edge has a stable semantic ID. Rectangles expose `front`, `left`, and `right`; L-shapes additionally expose `notch-horizontal` and `notch-vertical`. Each `stairSystems[]` entry has a stable ID, lock state, exact edge ID, attachment offset, width, tread depth, maximum-riser intent, and ordered `landings[]`. Each landing has its own stable ID, lock state, `afterRiser`, width, depth, `straight | left | right` primary turn, and optional ordered `connections[]`. A connection records a stable ID, lock state, deck- or grade-bound destination, open landing side, width, and tread depth. A deck-bound connection may additionally record another platform's stable ID; normalization rejects missing, same-platform, grade-bound, and same-elevation targets. The route uses that platform's exact elevation and divides the rise evenly so the final tread terminates at that height. It deliberately does not infer a target edge or horizontal endpoint: the UI marks alignment for review rather than pretending two spatially separate levels connect. This makes the landing an explicit merger junction: multiple flights may converge or diverge there while the physical landing, supports, and quantity are projected once. Each used side removes only that side's landing guard, so no railing crosses a recorded stair opening. Multiple unfinished landings may now be added and positioned before their details are finished; shared-flight connections still require an explicitly finished landing. Another stair system is started only after the active system is finished. Systems on the same deck edge cannot overlap, and unrelated geometry is never silently assigned to a merger. Landing order, rise bounds, unique open sides, width, and turn-depth constraints fail closed. The same route engine drives 2D, 3D, stringers, stair railings, landing protection, and quantities. Undo/redo restores the entire grouped fact change as a new monotonic revision rather than silently moving the authoritative revision backward.
...
Multiple platforms are presented as selectable level layers. `Selected layer` view hides other platforms for focused editing; `Combined view` projects every level and exposes a whole-layer placement handle for the selected platform. Whole-layer movement translates its outer region and every cutout together through the same safe region-replacement boundary, with exact left/right and distance-from-house fields retained for fine adjustment. These view choices are browser presentation state and never enter DeckDesign. Adding a level requires an explicit measured height above grade and starts the new platform horizontally stacked on the source rather than inventing either elevation or a side offset. Explicit platform elevations from 6 through 360 inches are accepted as conceptual design facts.
...
The editor shell and deterministic 2D/quantity projections load independently of the Three.js runtime. The 3D view, guided house-connection editor, level/cutout controls, platform commands, and shared-landing connection controls are lazy browser chunks with visible status fallbacks. A post-build check enforces gzip budgets of 99 KiB for the initial entry, 170 KiB for the largest JavaScript chunk, and 241 KiB for all JavaScript combined. The exact level-side connection and measured multi-level setup slice measures 97.4 KiB initial and 240.6 KiB total. The total ceiling moved by one KiB while the initial entry became smaller, retaining a narrow regression gate for the added validation and setup paths. Local economy, balanced, and detailed tiers bound pixel ratio and shadow cost without entering the authoritative design document or changing projections. These are prototype regression limits, not production service-level guarantees; later device testing can tighten them.
