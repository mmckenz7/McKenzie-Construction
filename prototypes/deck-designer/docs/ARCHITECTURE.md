...
The review keeps each warning's stable `geometryIds` in memory. A contextual Show in plan action resolves those IDs only against the current normalized platform, fails closed for stale references, and selects the matching cutout, beam line, edge, or stair using existing editor state. This locator adds no authoritative design facts and is excluded from JSON, storage, quantities, and future adapter semantics.
