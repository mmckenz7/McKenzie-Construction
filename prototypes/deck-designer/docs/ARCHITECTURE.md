...
The v3 framing projection also derives one conceptual main beam line from existing `beamInset`, board/joist direction, and the normalized polygon region. `beamInset` is the exact perpendicular distance from the polygon's outside-axis bound; the editor exposes it in feet and through a touch-sized plan handle. Dragging follows the selected snap increment, exact entry remains unsnapped, placement stays within the usable half of the platform, and each committed move is a normal revision with Undo/Redo. The projection clips the beam into exact intervals around concave boundaries and recorded holes, then places conceptual support locations at segment endpoints and in bays no greater than the recorded `maxPostSpacing`. This framing fact does not alter deck-railing post geometry or quantities. Rotating the board direction rotates joists and the perpendicular support line together. The plan, 3D model, and separately classified visualization quantities consume these same beam/post IDs. This does not determine beam size, ply count, span, footing, connection, soil, code, or structural adequacy.
