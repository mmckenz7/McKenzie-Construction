...
The editor shell and deterministic 2D/quantity projections load independently of the Three.js runtime. The 3D view, guided house-connection editor, and shared-landing connection controls are lazy browser chunks with visible status fallbacks. A post-build check enforces gzip budgets of 96 KiB for the initial entry, 170 KiB for the largest JavaScript chunk, and 234 KiB for all JavaScript combined. The shared-junction slice measured 95.7 KiB initial and 233.8 KiB total; the advanced connection controls load as a separate 1.17 KiB gzip chunk. The ceilings were raised only enough to retain a measurable regression gate for the added authoritative facts, route projection, and validation. Local economy, balanced, and detailed tiers bound pixel ratio and shadow cost without entering the authoritative design document or changing projections. These are prototype regression limits, not production service-level guarantees; later device testing can tighten them.
