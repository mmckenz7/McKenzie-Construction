...
import { deriveDeckDesignProjectionV5, type DeckPlatformGeometrySourceV5 } from "./designProjectionV5";
...
export function V5App({ initialDesign, initialMessage = "Corner editor ready.", startWithPhotos = false }: { initialDesign: DeckDesignV5; initialMessage?: string; startWithPhotos?: boolean }) {
  ...
  const designFingerprint = useMemo(() => deckDesignV5Fingerprint(design), [design]);
  ...
  const geometrySource = useMemo<DeckPlatformGeometrySourceV5>(() => Object.freeze([designFingerprint, geometry]), [designFingerprint, geometry]);
  const houseGeometry = useMemo(() => deriveHouseContextGeometry(design.siteContext), [design.siteContext]);
  const projection = useMemo(() => deriveDeckDesignProjectionV5(design, geometrySource), [design, geometrySource]);
  ...
  const [renderPalette, setRenderPalette] = useState<RenderPalette>("cedar");
  ...
  const keepSelectedLevelOnly = () => {
    ...
    apply(normalizeDeckDesignV5({ ...history.present, platforms: [kept], metadata: { ...history.present.metadata, revision: history.present.metadata.revision + 1 } }), "Single level restored; links removed.");
  ...
  const addCutout = () => {
    try {
      ...
      if (width < 12 || depth < 12) throw new RangeError("This level is too small for that cutout.");
  ...
  const addStairsToEdge = (edgeId: string, edgeLength: number) => {
    if (!platform.edgeConditions.some((condition) => condition.edgeId === edgeId && condition.condition === "free")) { setMessage("Stairs require a free side."); return; }
    if (platform.construction.stairSystems.some((system) => system.edgeId === edgeId)) { setMessage("This side already has stairs."); return; }
  ...
  const addLanding = (kind: "top" | "midway") => {
    ...
    if (kind === "top" && existingTopLanding) { setSelectedLandingId(existingTopLanding.id); setMessage("Top landing already added."); return; }
    ...
    if (kind === "midway" && (totalRisers < 2 || (previousMidway && afterRiser <= previousMidway.afterRiser))) { setMessage("No stair rise remains."); return; }
  ...
  const addLandingConnection = () => {
    if (!activeStairSystem || !activeLanding?.locked) { setMessage("Lock the landing first."); return; }
    ...
    updateLanding({ connections: [...activeLanding.connections, connection] }, "Flight added to grade.");
  };
  const lockLanding = () => updateLanding({ locked: true }, "Landing locked.");
  const lockStairSystem = () => {
    if (!activeStairSystem) return;
    replaceStairSystems(platform.construction.stairSystems.map((system) => system.id === activeStairSystem.id ? lockStairData(system) : system), "Stairs locked.");
  ...
  const selectStairObject = (systemId: string, landingId?: string) => {
    ...
    if (!system) { setMessage("Stairs unavailable."); return; }
  ...
  return <main>
    ...
    {layoutReviewOpen && layoutReview && <div className="layout-review-backdrop" role="presentation"><section className="layout-review-dialog" role="dialog" aria-modal="true" aria-labelledby="layout-review-title" aria-describedby="layout-review-description" onKeyDown={handleLayoutReviewKeyDown}>
      <header><div><p className="eyebrow">Geometry check</p><h2 id="layout-review-title">Review deck layout</h2><p id="layout-review-description">Confirm geometry before railings.</p></div><button ref={layoutReviewClose} onClick={closeLayoutReview} aria-label="Close layout review">Close</button></header>
      <div className="layout-review-content">
        <div className="layout-review-status"><strong>{layoutReview.readyToContinue ? "Ready for railings" : "Finish highlighted geometry"}</strong><span>Conceptual · not for construction</span></div>
    ...
    <header className="topbar"><div className="brand-mark">M</div><div><p className="eyebrow">McKenzie Construction · isolated R&amp;D</p><h1>Deck Designer</h1></div><div className="header-actions"><button className="quiet" onClick={openPhotoIntake}>Start with photos</button><button className="quiet" onClick={() => { saveDeckDesignV5(localStorage, design); setMessage("Saved locally."); }}>Save locally</button><button className="quiet" onClick={download}>Download JSON</button><button className="primary" onClick={() => fileInput.current?.click()}>Load JSON</button><input ref={fileInput} hidden type="file" accept="application/json,.json" onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; try { const next = migrateDeckDesignToV5(JSON.parse(await file.text())); dispatch({ type: "reset", design: next }); setSelectedPlatformId(next.platforms[0].id); setWorkflowStage("layout"); clearPlanSelection(); setMessage("Design loaded."); } catch (error) { setMessage(error instanceof Error ? `Load rejected: ${error.message}` : "Load rejected."); } event.target.value = ""; }} /></div></header>
      <section className="warning"><strong>Conceptual—not for construction.</strong> Field review required.</section>
    ...
    <div className="workspace"><aside className="controls-panel" id="design-controls">
      ...
      <section className="selected-edge-card"><strong>Conceptual beams</strong><p>Select a beam.</p><div className="toggle-grid two">{platform.construction.framing.beamLines.map((line, index) => <button type="button" key={line.id} className={`toggle${line.id === selectedBeamLineId ? " active" : ""}`} aria-pressed={line.id === selectedBeamLineId} onClick={() => setSelectedBeamLineId(line.id)}>Beam {index + 1}</button>)}</div>{(() => { const beam = platform.construction.framing.beamLines.find((line) => line.id === selectedBeamLineId) ?? platform.construction.framing.beamLines[0]; return <><div className="field-grid"><V3NumberField label="From outside edge (feet)" value={Math.round(beam.offsetFromOutside / 12 * 100) / 100} step={.5} onCommit={(value) => moveBeam(value * 12)} /><V3NumberField label="Max support spacing (feet)" value={Math.round(beam.maxSupportSpacing / 12 * 100) / 100} step={.5} onCommit={(value) => { try { apply(updateBeamLineV5(history.present, platform.id, { ...beam, maxSupportSpacing: value * 12 }).design, "Support spacing updated."); } catch (error) { setMessage(error instanceof Error ? error.message : CHANGE_REJECTED); } }} /></div><div className="plan-action-buttons"><button disabled={platform.construction.framing.beamLines.length >= 6} onClick={() => { try { const suffix = Math.max(0, ...platform.construction.framing.beamLines.map((line) => Number(line.id.match(/\d+$/)?.[0] ?? 0))) + 1; const values = platform.region.outer.map((point) => platform.construction.decking.direction === "left_right" ? point.z : point.x); const span = Math.max(...values) - Math.min(...values); const occupied = new Set(platform.construction.framing.beamLines.map((line) => line.offsetFromOutside)); const offset = Array.from({ length: Math.floor((span - 12) / 6) }, (_, index) => 6 + index * 6).find((value) => !occupied.has(value)) ?? span - 6; const result = addBeamLineV5(history.present, platform.id, { id: `beam-line-${suffix}`, offsetFromOutside: offset, maxSupportSpacing: beam.maxSupportSpacing }); setSelectedBeamLineId(result.beamLineId); apply(result.design, "Beam added."); } catch (error) { setMessage(error instanceof Error ? error.message : CHANGE_REJECTED); } }}>Add beam</button><button disabled={platform.construction.framing.beamLines.length === 1} onClick={() => { try { const result = removeBeamLineV5(history.present, platform.id, beam.id); setSelectedBeamLineId(result.design.platforms.find((item) => item.id === platform.id)!.construction.framing.beamLines[0].id); apply(result.design, "Beam removed."); } catch (error) { setMessage(error instanceof Error ? error.message : CHANGE_REJECTED); } }}>Remove selected</button></div></>; })()}</section>
      <p className="level-height-note">Visualization—not structural approval.</p>
      <p className="outline-edit-feedback">{message}</p>
      <Suspense fallback={<div className="house-editor-loading" role="status">Loading house…</div>}><HouseConnectionEditor design={compatibilityDesign} platform={compatibilityPlatform} onApply={(next, attachment) => updateHouseConnection(restoreV5Authority(history.present, next), attachment)} onError={setMessage} /></Suspense>
      <section className="stage-continue-card"><span>Layout ready?</span><strong>Review before railings.</strong><button className="primary" onClick={requestRailingStage}>Review layout</button><small>Checks geometry.</small></section>
      </> : workflowStage === "railings" ? <><Suspense fallback={<div className="house-editor-loading" role="status">Loading railings…</div>}><RailingStageControls platform={compatibilityPlatform} geometry={geometry} selectedEdgeId={selectedEdgeId} onRailingChange={applyRailing} onHeight={(height) => updateConstruction({ ...platform.construction, railing: { ...platform.construction.railing, height } }, "Railing height updated.")} onBack={returnToLayoutStage} /></Suspense><section className="stage-continue-card"><span>Railings ready?</span><strong>Choose edge finishes.</strong><button className="primary" onClick={enterFinishStage}>Continue to finishes</button><small>Products and pricing stay separate.</small></section></> : <Suspense fallback={<div className="house-editor-loading" role="status">Loading finishes…</div>}><FinishStageControls platform={platform} geometry={geometry} selectedEdgeId={selectedEdgeId} gradeElevation={design.siteContext.gradeElevation} onChange={applyFinish} onBack={returnToRailingStage} /></Suspense>}
      <div className="history-actions"><button disabled={!history.past.length} onClick={() => changeHistory("undo")}>Undo</button><button disabled={!history.future.length} onClick={() => changeHistory("redo")}>Redo</button></div>
       ...
       <article className="view-card plan-card"><div className="card-title"><div><span>{workflowStage === "layout" ? "Measured plan" : workflowStage === "railings" ? "Railing plan" : "Finish plan"}</span><small>{workflowStage === "layout" ? `2D · ${platform.id} · ${geometry.platformEdges.length} sides` : workflowStage === "railings" ? "2D · tap a railing side" : "2D · tap a finish side"}</small></div><div className="plan-card-tools"><strong>{formatFeetInches(platform.elevation)} above grade</strong><div className="plan-history-actions"><button disabled={!history.past.length} onClick={() => changeHistory("undo")}>Undo</button><button disabled={!history.future.length} onClick={() => changeHistory("redo")}>Redo</button></div></div></div><PlanViewV3 platform={compatibilityPlatform} activeStairSystem={activeStairSystem} geometry={visibleGeometry} houseGeometry={houseGeometry} snapIncrement={snapIncrement} editingEnabled={workflowStage === "layout"} outlineEditingEnabled={!hasOutlineOptionLocks} selectedEdgeId={selectedEdgeId} selectedHoleIndex={selectedHoleIndex} selectedLandingId={selectedLandingId} beamLines={platform.construction.framing.beamLines} selectedBeamLineId={selectedBeamLineId} onSelectBeamLine={setSelectedBeamLineId} onSelectEdge={workflowStage === "layout" ? selectLayoutEdge : setSelectedEdgeId} onSelectStairSystem={(systemId) => selectStairObject(systemId)} onSelectLanding={(systemId, landingId) => selectStairObject(systemId, landingId)} onSelectHouseOpening={selectHouseOpening} onSelectHole={(index) => { setSelectedHoleIndex(index); setSelectedEdgeId(null); setSelectedStairSystemId(null); setSelectedLandingId(null); setMessage(`Cutout ${index + 1} selected. Drag center or corner.`); }} onHolePreview={(index, hole) => moveCutout(index, hole, false)} onHoleCommit={(index, hole) => moveCutout(index, hole, true)} onCornerPreview={(index, point) => moveCorner(index, point, false)} onCornerCommit={(index, point, magnetic) => moveCorner(index, point, true, magnetic)} onCancel={() => setPreview(null)} onStairPreview={(offset) => moveActiveStairs(offset, "", true)} onStairCommit={(offset) => moveActiveStairs(offset, `Stairs moved to ${formatFeetInches(offset)}.`)} onSegmentPreview={(index, distance) => moveSegment(index, distance, false)} onSegmentCommit={(index, distance) => moveSegment(index, distance, true)} onBeamPreview={(inset) => moveBeam(inset, true)} onBeamCommit={(inset) => moveBeam(inset)} />
        {workflowStage === "layout" ? <section ref={planActionTray} className={`plan-action-tray${activeStairSystem ? " editing-stairs" : ""}`} aria-live="polite">
          {hasOutlineOptionLocks && !activeStairSystem ? <div className="plan-action-copy outline-locked-action"><strong>Unlock outline editing</strong><small>Clears side options; house stays fixed.</small><button className="primary" onClick={unlockOutline}>Unlock outline editing</button></div> : selectedEdgeId ? (() => {
            const edge = geometry.platformEdges.find((item) => item.id === selectedEdgeId);
            if (!edge) return <div className="plan-action-copy"><strong>Select a blueprint side</strong><small>Its controls appear here.</small></div>;
              ...
              {!activeStairSystem ? <div className="plan-action-buttons"><button disabled={!isFree} onClick={() => addBumpoutToEdge(edge.id)}>{isFree ? "Add bumpout" : "House side stays fixed"}</button><button className="primary" disabled={!isFree || edge.length < 48} onClick={() => addStairsToEdge(edge.id, edge.length)}>Add stairs</button></div> : activeStairSystem.locked ? <div className="plan-action-buttons"><button onClick={() => updateStairSystem({ locked: false }, "Stairs reopened for editing.")}>Edit stairs</button><button className="primary" onClick={() => { setSelectedEdgeId(null); setSelectedStairSystemId(null); setSelectedLandingId(null); }}>Close side</button></div> : <>
                ...
                {activeLanding && <div className="stair-landing-controls segment-landing-controls">
                  ...
                  <div className="field-grid"><V3NumberField label="Landing width (feet)" value={Math.round(activeLanding.width / 12 * 100) / 100} step={.5} onCommit={(value) => updateLanding({ width: value * 12 }, "Landing width updated exactly.")} /><V3NumberField label="Landing depth (feet)" value={Math.round(activeLanding.depth / 12 * 100) / 100} step={.5} onCommit={(value) => updateLanding({ depth: value * 12 }, "Landing depth updated exactly.")} /></div>
                   {!activeLanding.terminalPlatformId && <fieldset><legend>Direction after landing</legend><div className={`toggle-grid${activeLanding.afterRiser > 0 ? " four" : ""}`}>{(activeLanding.afterRiser > 0 ? ["straight", "left", "right", "switchback"] as const : ["straight", "left", "right"] as const).map((turn) => <button type="button" key={turn} className={`toggle${activeLanding.turn === turn ? " active" : ""}`} aria-pressed={activeLanding.turn === turn} onClick={() => updateLanding({ turn, afterRiser: turn === "switchback" ? Math.max(activeLanding.afterRiser, Math.ceil(activeTotalRisers / 2)) : activeLanding.afterRiser, width: turn === "switchback" ? Math.max(activeLanding.width, activeStairSystem.width * 2) : activeLanding.width, depth: turn === "straight" ? activeLanding.depth : Math.max(activeLanding.depth, activeStairSystem.width) }, `Stairs continue ${turn}.`)}>{turn}</button>)}</div><small>Viewed walking down.</small></fieldset>}
                  {!activeLanding.locked && <button className="primary" onClick={lockLanding}>Finish landing details</button>}
                   {activeLanding.locked && !activeLanding.terminalPlatformId && <Suspense fallback={<div className="house-editor-loading" role="status">Loading flights…</div>}><LandingConnectionsEditor landing={activeLanding} onAdd={addLandingConnection} onUpdateLanding={(connections, nextMessage) => updateLanding({ connections }, nextMessage)} onUpdateConnection={updateLandingConnection} /></Suspense>}
                </div>}
          ...
          })() : <div className="plan-action-copy"><strong>Select a blueprint side</strong><small>Its controls appear here.</small></div>}
      ...
      <article className="view-card three-card"><div className="card-title"><div><span>{workflowStage === "layout" ? "Model view" : workflowStage === "railings" ? "Railing model" : "Finish model"}</span><small>3D · single level</small></div><div className="view-tools"><select value={renderPalette} aria-label="3D deck color" onChange={(event) => setRenderPalette(event.target.value as RenderPalette)}><option value="cedar">Cedar</option><option value="brown">Brown</option><option value="gray">Gray</option></select><select value={quality} aria-label="3D quality" onChange={(event) => setQuality(event.target.value as RenderQuality)}><option value="economy">Economy</option><option value="balanced">Balanced</option><option value="detailed">Detailed</option></select><div className="camera-buttons">{(["perspective", "top", "front"] as CameraPreset[]).map((value) => <button key={value} className={preset === value ? "active" : ""} aria-pressed={preset === value} onClick={() => { setPreset(value); setPresetRequest((current) => current + 1); }}>{value}</button>)}</div></div></div><Suspense fallback={<div className="three-loading">Preparing model…</div>}><ThreeViewV3 platform={compatibilityPlatform} geometry={visibleGeometry} houseGeometry={houseGeometry} gradeElevation={design.siteContext.gradeElevation} preset={preset} presetRequest={presetRequest} showFraming={showFraming} quality={quality} palette={renderPalette} /></Suspense><label className="check-row three-framing"><input type="checkbox" checked={showFraming} onChange={(event) => setShowFraming(event.target.checked)} />Show framing</label></article>
    </section></div>
     <section className="quantity-section"><div className="quantity-heading"><div><p className="eyebrow">Deterministic v5</p><h2>Conceptual quantities</h2></div><p>{photoStartSummary?.review.outlineWarning ? "Envelope quantities." : "No prices or structural claims."}</p></div><div className="quantity-grid">{projection.aggregateQuantities.map((line) => <article className="quantity-card" key={line.key}><span>{line.key.replaceAll("-", " ")}</span><strong>{line.amount.toLocaleString()} <small>{line.unit}</small></strong><p>{line.sourceGeometry.length} geometry refs.</p></article>)}</div></section>
  </main>;
}
