...
export function LandingConnectionsEditor({ landing, onAdd, onUpdateLanding, onUpdateConnection }: Readonly<{
  ...
  return <section className="landing-connections">
    <div><strong>Landing flights</strong><small>Add another flight to grade.</small></div>
    {landing.connections.map((connection, index) => <article className="landing-connection-card" key={connection.id}>
      ...
      {!connection.locked && <>
        {connection.destination === "deck" && <button type="button" onClick={() => onUpdateConnection(connection.id, { destination: "grade", targetPlatformId: undefined, targetEdgeId: undefined }, "Flight changed to grade.")}>Change to ground</button>}
        ...
        <div className="field-grid"><V3NumberField label="Connected stair width" value={connection.width} onCommit={(value) => onUpdateConnection(connection.id, { width: value }, "Connected stair width updated exactly.")} /><V3NumberField label="Connected step depth" value={connection.treadDepth} onCommit={(value) => onUpdateConnection(connection.id, { treadDepth: value }, "Connected step depth updated exactly.")} /></div>
        <button className="primary" disabled={connection.destination !== "grade"} onClick={() => onUpdateConnection(connection.id, { locked: true }, "Flight locked to grade.")}>Finish connection</button>
      </>}
      <button onClick={() => onUpdateLanding(landing.connections.filter((other) => other.id !== connection.id), "Flight removed; landing retained.")}>Remove connected flight</button>
    </article>)}
    ...
    <small>Directions face down-stair; house sides stay excluded.</small>
  </section>;
}
