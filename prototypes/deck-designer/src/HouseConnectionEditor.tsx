...
export function HouseConnectionEditor({ design, platform, onApply, onError }: Props) {
  ...
  return <section className="house-connection-editor" id="house-connection">
    ...
    <div className="house-wall-selector" aria-label="Recorded house walls">{design.siteContext.houseWalls.map((wall, index) => <button key={wall.id} type="button" aria-pressed={wallId === wall.id} onClick={() => setWallId(wall.id)}>Wall {index + 1}</button>)}<button type="button" aria-pressed={addingWall} disabled={design.siteContext.houseWalls.length >= 8} onClick={() => { setWallId("new"); setEdgeId(""); setAttachment("unknown"); setDoorEnabled(false); }}>{design.siteContext.houseWalls.length < 2 ? "Add second wall" : "Add another wall"}</button></div>
