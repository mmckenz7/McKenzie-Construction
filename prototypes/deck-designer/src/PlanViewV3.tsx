...
export function PlanViewV3({ platform, activeStairSystem = null, geometry, houseGeometry, snapIncrement, editingEnabled = true, selectedEdgeId, selectedHoleIndex = null, selectedLandingId = null, contextPlatforms = [], platformMoveEnabled = false, onSelectEdge, onSelectStairSystem, onSelectLanding, onSelectHouseOpening, onSelectHole, onSelectContextPlatform, onPlatformPreview, onPlatformCommit, onHolePreview, onHoleCommit, onCornerPreview, onCornerCommit, onCancel, onStairPreview, onStairCommit, onSegmentPreview, onSegmentCommit, onBeamPreview, onBeamCommit, beamLines, selectedBeamLineId, onSelectBeamLine }: Props) {
  ...
  const stairPointer = (event: PointerEvent<SVGCircleElement>, commit: boolean): boolean => {
    const point = localPoint(event), drag = stairDrag.current;
    if (!point || !drag || !stairEdge || !stair) return false;
    ...
    if (offset === stair.offset) return false;
  ...
  const cancelHoleDrag = () => { if (!holeDrag.current) return; holeDrag.current = null; setActive(null); onCancel(); };
  ...
  const beamPointer = (event: PointerEvent<SVGCircleElement>, commit: boolean): boolean => {
  ...
  const cancelBeamDrag = () => { if (!beamDrag.current) return; beamDrag.current = null; setActive(null); onCancel(); };
  ...
  return <svg ref={ref} className="plan-svg v3-plan" viewBox={`0 0 ${maxX - minX + margin * 2} ${maxZ - minZ + margin * 2}`} role="group" aria-label={editingEnabled ? `Editable ${geometry.footprint.length}-corner deck outline` : `Side selection plan with ${geometry.platformEdges.length} deck sides`} aria-describedby={instructionsId}>
    <desc id={instructionsId}>{editingEnabled ? "Tab through deck objects and movement handles. Press Enter or Space to select an object. Use arrow keys on a focused movement handle for exact snapped changes." : "Tab through the deck sides. Press Enter or Space to select a side."}</desc>
    ...
    {editingEnabled && geometry.stairTreads.map((tread) => <polygon key={`select-${tread.id}`} points={tread.corners.map((p) => `${x(p.x)},${y(p.z)}`).join(" ")} className="plan-object-hit" role="button" tabIndex={0} aria-label={`Edit ${tread.systemId.replaceAll("-", " ")}`} aria-pressed={activeStairSystem?.id === tread.systemId} onClick={() => onSelectStairSystem?.(tread.systemId)} onKeyDown={(event) => { if (event.key !== "Enter" && event.key !== " ") return; event.preventDefault(); onSelectStairSystem?.(tread.systemId); }} />)}
    {editingEnabled && geometry.platformEdges.map((edge, index) => {
      const midpoint = { x: (edge.start.x + edge.end.x) / 2, z: (edge.start.z + edge.end.z) / 2 };
      return <g key={`segment-${index}`}><rect x={x(midpoint.x) - 18} y={y(midpoint.z) - 18} width="36" height="36" rx="8" className="segment-move-hit" role="button" tabIndex={0} aria-label={`Move ${planEdgeDimensionLabel(edge).text} side; drag or use arrow keys perpendicular to reshape attached sides; snaps to ${formatFeetInches(snapIncrement)}`} onKeyDown={(event) => { const directions: Record<string, Point> = { ArrowLeft: { x: -1, z: 0 }, ArrowRight: { x: 1, z: 0 }, ArrowUp: { x: 0, z: -1 }, ArrowDown: { x: 0, z: 1 } }; const direction = directions[event.key]; if (!direction) return; const projection = direction.x * edge.outward.x + direction.z * edge.outward.z; if (Math.abs(projection) < .5) return; event.preventDefault(); onSelectEdge(edge.id); onSegmentCommit(index, Math.sign(projection) * snapIncrement); }} onPointerDown={(event) => { const start = localPoint(event); if (!start) return; onSelectEdge(edge.id); event.currentTarget.setPointerCapture(event.pointerId); segmentDrag.current = { index, start, outward: edge.outward }; setActive(`segment-${index}`); }} onPointerMove={(event) => { if (active !== `segment-${index}` || !event.currentTarget.hasPointerCapture(event.pointerId)) return; const distance = segmentDistance(event); if (distance !== null && Math.abs(distance) > 1e-6) onSegmentPreview(index, distance); }} onPointerUp={(event) => { if (active !== `segment-${index}`) return; const distance = segmentDistance(event); if (distance !== null && Math.abs(distance) > 1e-6) onSegmentCommit(index, distance); else onCancel(); event.currentTarget.releasePointerCapture(event.pointerId); segmentDrag.current = null; setActive(null); }} onPointerCancel={() => { segmentDrag.current = null; setActive(null); onCancel(); }} /><rect x={x(midpoint.x) - 4.5} y={y(midpoint.z) - 4.5} width="9" height="9" rx="2" className={`segment-move-handle${active === `segment-${index}` ? " active" : ""}`} aria-hidden="true" pointerEvents="none" /></g>;
    })}
    {editingEnabled && selectedHole && selectedHoleIndex !== null && selectedHoleCenter && <>
      <rect x={x(selectedHoleCenter.x) - 18} y={y(selectedHoleCenter.z) - 18} width="36" height="36" rx="8" className="cutout-move-hit" role="button" tabIndex={0} aria-label={`Move cutout ${selectedHoleIndex + 1}`} onKeyDown={(event) => nudgeHole("move", event)} onPointerDown={(event) => { const start = localPoint(event); if (!start) return; event.currentTarget.setPointerCapture(event.pointerId); holeDrag.current = { index: selectedHoleIndex, mode: "move", start, hole: selectedHole }; setActive("hole-move"); }} onPointerMove={(event) => { const drag = holeDrag.current; if (drag?.index !== selectedHoleIndex || drag.mode !== "move" || !event.currentTarget.hasPointerCapture(event.pointerId)) return; const hole = draggedHole(event); if (holeChanged(hole, drag.hole)) onHolePreview?.(selectedHoleIndex, hole); }} onPointerUp={(event) => { const drag = holeDrag.current; if (drag?.index !== selectedHoleIndex || drag.mode !== "move") return; const hole = draggedHole(event); holeDrag.current = null; if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); setActive(null); if (holeChanged(hole, drag.hole)) onHoleCommit?.(selectedHoleIndex, hole); else onCancel(); }} onPointerCancel={cancelHoleDrag} onLostPointerCapture={cancelHoleDrag} />
      ...
      {selectedHole.map((point, cornerIndex) => <g key={`hole-handle-${cornerIndex}`}><circle cx={x(point.x)} cy={y(point.z)} r="16" className="cutout-resize-hit" role="button" tabIndex={0} aria-label={`Resize cutout ${selectedHoleIndex + 1} from corner ${cornerIndex + 1}`} onKeyDown={(event) => nudgeHole(cornerIndex, event)} onPointerDown={(event) => { const start = localPoint(event); if (!start) return; event.currentTarget.setPointerCapture(event.pointerId); holeDrag.current = { index: selectedHoleIndex, mode: cornerIndex, start, hole: selectedHole }; setActive(`hole-corner-${cornerIndex}`); }} onPointerMove={(event) => { const drag = holeDrag.current; if (drag?.index !== selectedHoleIndex || drag.mode !== cornerIndex || !event.currentTarget.hasPointerCapture(event.pointerId)) return; const hole = draggedHole(event); if (holeChanged(hole, drag.hole)) onHolePreview?.(selectedHoleIndex, hole); }} onPointerUp={(event) => { const drag = holeDrag.current; if (drag?.index !== selectedHoleIndex || drag.mode !== cornerIndex) return; const hole = draggedHole(event); holeDrag.current = null; if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); setActive(null); if (holeChanged(hole, drag.hole)) onHoleCommit?.(selectedHoleIndex, hole); else onCancel(); }} onPointerCancel={cancelHoleDrag} onLostPointerCapture={cancelHoleDrag} /><circle cx={x(point.x)} cy={y(point.z)} r="5.5" className="cutout-resize-handle" aria-hidden="true" pointerEvents="none" /></g>)}
    </>}
    {editingEnabled && platform.region.outer.map((point, index) => <g key={index}><circle cx={x(point.x)} cy={y(point.z)} r="18" className="corner-move-hit" role="button" tabIndex={0} aria-label={`Editable corner at ${point.x} inches left/right and ${point.z} inches away; drag or use arrow keys`} onKeyDown={(event) => nudgeCorner(index, event)} onPointerDown={(event) => { const start = localPoint(event); if (!start) return; cornerDrag.current = { index, start, corner: point }; event.currentTarget.setPointerCapture(event.pointerId); setActive(`corner-${index}`); }} onPointerMove={(event) => { if (active === `corner-${index}` && event.currentTarget.hasPointerCapture(event.pointerId)) { const moved = draggedCorner(event); if (moved && (moved.x !== point.x || moved.z !== point.z)) onCornerPreview(index, moved); } }} onPointerUp={(event) => { if (active !== `corner-${index}`) return; const moved = draggedCorner(event); if (moved && (moved.x !== point.x || moved.z !== point.z)) onCornerCommit(index, moved); else onCancel(); event.currentTarget.releasePointerCapture(event.pointerId); cornerDrag.current = null; setActive(null); }} onPointerCancel={() => { cornerDrag.current = null; setActive(null); onCancel(); }} /><circle cx={x(point.x)} cy={y(point.z)} r={active === `corner-${index}` ? 7 : 5.5} className="dimension-handle corner-handle" aria-hidden="true" pointerEvents="none" /></g>)}
    {editingEnabled && stair && !stair.locked && stairCenter && <g><circle cx={x(stairCenter.x)} cy={y(stairCenter.z)} r="18" className="stair-move-hit" role="button" tabIndex={0} aria-label={`Move ${stair.id}, currently ${formatFeetInches(stair.offset)} from the start of its selected side; drag or use arrow keys; snaps to ${formatFeetInches(snapIncrement)}`} onKeyDown={nudgeStairs} onPointerDown={(event) => { const start = localPoint(event); if (!start) return; stairDrag.current = { start, offset: stair.offset }; event.currentTarget.setPointerCapture(event.pointerId); setActive("stairs"); }} onPointerMove={(event) => { if (active === "stairs" && event.currentTarget.hasPointerCapture(event.pointerId)) stairPointer(event, false); }} onPointerUp={(event) => { if (active !== "stairs") return; if (!stairPointer(event, true)) onCancel(); event.currentTarget.releasePointerCapture(event.pointerId); stairDrag.current = null; setActive(null); }} onPointerCancel={() => { stairDrag.current = null; setActive(null); onCancel(); }} /><circle cx={x(stairCenter.x)} cy={y(stairCenter.z)} r={active === "stairs" ? 8 : 6.5} className="stair-move-handle" aria-hidden="true" pointerEvents="none" /></g>}
    {editingEnabled && beamHandle && <><circle cx={x(beamHandle.x)} cy={y(beamHandle.z)} r="15" className="beam-move-hit" role="button" tabIndex={0} aria-label={`Move conceptual beam, currently ${formatFeetInches(selectedBeam?.offsetFromOutside ?? effectiveBeamInsetV3(platform))} from the outside edge`} onKeyDown={(event) => { const increaseKeys = platform.construction.decking.direction === "left_right" ? ["ArrowUp"] : ["ArrowLeft"]; const decreaseKeys = platform.construction.decking.direction === "left_right" ? ["ArrowDown"] : ["ArrowRight"]; if (!increaseKeys.includes(event.key) && !decreaseKeys.includes(event.key)) return; event.preventDefault(); const direction = increaseKeys.includes(event.key) ? 1 : -1; onBeamCommit?.((selectedBeam?.offsetFromOutside ?? effectiveBeamInsetV3(platform)) + direction * snapIncrement); }} onPointerDown={(event) => { const start = localPoint(event); if (!start) return; if (selectedBeam) onSelectBeamLine?.(selectedBeam.id); event.currentTarget.setPointerCapture(event.pointerId); beamDrag.current = { start, inset: selectedBeam?.offsetFromOutside ?? effectiveBeamInsetV3(platform) }; setActive("beam-move"); }} onPointerMove={(event) => { if (!beamDrag.current || !event.currentTarget.hasPointerCapture(event.pointerId)) return; beamPointer(event, false); }} onPointerUp={(event) => { if (!beamDrag.current) return; const changed = beamPointer(event, true); beamDrag.current = null; if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); setActive(null); if (!changed) onCancel(); }} onPointerCancel={cancelBeamDrag} onLostPointerCapture={cancelBeamDrag} /><rect x={x(beamHandle.x) - 5} y={y(beamHandle.z) - 5} width="10" height="10" rx="2" className={`beam-move-handle${active === "beam-move" ? " active" : ""}`} aria-hidden="true" pointerEvents="none" /></>}
