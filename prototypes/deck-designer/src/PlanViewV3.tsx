...
export function PlanViewV3({ platform, geometry, snapIncrement, selectedEdgeId, onSelectEdge, onCornerPreview, onCornerCommit, onCancel, onStairPreview, onStairCommit, addCornerMode, onAddCorner, onSegmentPreview, onSegmentCommit }: Props) {
  ...
  return <svg ref={ref} className={`plan-svg v3-plan${addCornerMode ? " add-corner-mode" : ""}`} viewBox={`0 0 ${maxX - minX + margin * 2} ${maxZ - minZ + margin * 2}`} role="img" aria-label={`Editable ${geometry.footprint.length}-corner deck outline`}>
    ...
    {geometry.platformEdges.map((edge, index) => {
      ...
      return <g key={edge.id}>{selectedEdgeId === edge.id && <line x1={x(edge.start.x)} y1={y(edge.start.z)} x2={x(edge.end.x)} y2={y(edge.end.z)} className="plan-selected-edge" />}<polygon points={hitPoints.map((point) => `${x(point.x)},${y(point.z)}`).join(" ")} className="v3-edge" role="button" tabIndex={0} aria-label={addCornerMode ? `Add corner on segment ${index + 1}` : `Select segment ${index + 1}`} onClick={(event) => addCornerMode ? addCornerFromClick(index, event) : onSelectEdge(edge.id)} onKeyDown={(event) => { if (event.key !== "Enter" && event.key !== " ") return; event.preventDefault(); if (addCornerMode) onAddCorner(index, midpoint); else onSelectEdge(edge.id); }} /></g>;
    })}
