    ...
    <section className="trace-plan">
      ...
      <svg ref={svg} viewBox={`0 0 ${view.width} ${view.height}`} role="img" aria-label={`Photo-reference outline with ${outer.length} corners`} onPointerDownCapture={onPlanPointerDownCapture} onPointerMoveCapture={onPlanPointerMoveCapture} onPointerUpCapture={onPlanPointerEndCapture} onPointerCancelCapture={onPlanPointerEndCapture}>
        ...
        {edges.map((edge, index) => {
          ...
          return <g key={`trace-edge-${index}`}><line x1={x(edge.start.x)} y1={y(edge.start.z)} x2={x(edge.end.x)} y2={y(edge.end.z)} className={`${index === houseEdgeIndex ? "trace-house-edge" : "trace-edge"}${selection?.kind === "segment" && selection.index === index ? " highlighted" : ""}`} /><line x1={x(edge.start.x)} y1={y(edge.start.z)} x2={x(edge.end.x)} y2={y(edge.end.z)} className="trace-edge-hit" role="button" tabIndex={0} aria-label={`Select ${segmentDescription(edge)}`} onClick={() => setSelection({ kind: "segment", index })} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelection({ kind: "segment", index }); } }} /><rect x={x(labelX) - 36} y={y(labelZ) - 10} width="72" height="20" rx="5" className="trace-dimension-hit" role="button" tabIndex={0} aria-label={`Edit length of ${segmentDescription(edge)}`} onClick={(event) => { event.stopPropagation(); setSelection({ kind: "segment", index }); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelection({ kind: "segment", index }); } }} transform={`rotate(${labelAngle} ${x(labelX)} ${y(labelZ)})`} /><text x={x(labelX)} y={y(labelZ)} transform={`rotate(${labelAngle} ${x(labelX)} ${y(labelZ)})`} className="trace-dimension-label">{formatLength(edge.length)}</text>{index !== houseEdgeIndex && (() => {
