  ...
  return <div className="photo-trace-workspace">
    ...
    <section className="trace-plan">
      ...
      {(selectedCorner || selectedEdge) && <div className="trace-dimension-editor">
        ...
        {selectedEdge && selection?.kind === "segment" && (() => { const horizontal = Math.abs(selectedEdge.end.x - selectedEdge.start.x) >= Math.abs(selectedEdge.end.z - selectedEdge.start.z); const position = horizontal ? selectedEdge.start.z : selectedEdge.start.x; return <><div><strong>Edge {selection.index + 1} · {formatLength(selectedEdge.length)}</strong><small>Drag the white square to move both end corners together, or enter its exact plan position.</small></div><label><span>{horizontal ? "Away from house" : "Along house"} (ft)</span><input type="number" step="0.5" value={feet(position)} onFocus={() => remember(outer)} onChange={(event) => { if (event.target.value !== "") setSegmentFeet(Number(event.target.value)); }} /></label></>; })()}
      </div>}
      ...
      <p><span className="trace-dot round" /> Round corners move freely. <span className="trace-dot square" /> White squares move both ends of a segment together. Exact feet controls appear after selection.</p>
    </section>
  </div>;
}
