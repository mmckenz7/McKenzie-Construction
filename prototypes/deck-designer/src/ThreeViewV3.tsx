...
export function ThreeViewV3({ platform, geometry, houseGeometry, gradeElevation, preset, presetRequest, showFraming, quality }: Props) {
  ...
  useEffect(() => {
    ...
    for (const segment of geometry.landingRailSegments) { const landingY = geometry.landing?.y ?? platform.elevation; member(model, segment, landingY + platform.construction.railing.height - 2, 3, 2.5, rail); member(model, segment, landingY + 7, 2, 2, rail); }
    for (const post of [...geometry.railPosts, ...geometry.landingRailPosts]) { const mesh = new THREE.Mesh(new THREE.BoxGeometry(4, platform.construction.railing.height, 4), rail); mesh.position.set(post.x, post.top - platform.construction.railing.height / 2, post.z); mesh.castShadow = true; model.add(mesh); }
    ...
    if (geometry.landing) { const mesh = new THREE.Mesh(new THREE.BoxGeometry(geometry.landing.width, 5.5, geometry.landing.depth), deck); mesh.position.set(geometry.landing.center.x, geometry.landing.y - 2.25, geometry.landing.center.z); mesh.rotation.y = geometry.landing.rotationY; model.add(mesh); }
