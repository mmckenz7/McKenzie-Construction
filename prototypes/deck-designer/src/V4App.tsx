...
import { deriveDeckDesignProjectionV4 } from "./designProjectionV4";
import { derivePlatformGeometryV4 } from "./geometryV4";
import { createHistoryV4, designHistoryReducerV4 } from "./historyV4";
...
import { deckDesignV4Fingerprint, deckDesignV4ToV3Compatibility, migrateDeckDesignToV4, normalizeDeckDesignV4, stableDeckDesignV4Json, type DeckDesignV4, type DeckPlatformV4 } from "./modelV4";
import { PlanViewV3 } from "./PlanViewV3";
import { formatFeetInches } from "./PlanView";
import { saveDeckDesignV4 } from "./storageV4";
...
function revisePlatform(design: DeckDesignV4, platform: DeckPlatformV4): DeckDesignV4 {
  return normalizeDeckDesignV4({ ...design, platforms: design.platforms.map((item) => item.id === platform.id ? platform : item), metadata: { ...design.metadata, revision: design.metadata.revision + 1 } });
}

export function restoreV4BeamLines(previous: DeckDesignV4, next: DeckDesignV3): DeckDesignV4 {
  const migrated = migrateDeckDesignToV4(next);
  return normalizeDeckDesignV4({ ...migrated, platforms: migrated.platforms.map((platform) => {
    const prior = previous.platforms.find((candidate) => candidate.id === platform.id);
    return prior ? { ...platform, construction: { ...platform.construction, framing: { joistSpacing: platform.construction.framing.joistSpacing, beamLines: prior.construction.framing.beamLines } } } : platform;
  }) });
}

export function V4App({ initialDesign, initialMessage = "Corner editor ready.", startWithPhotos = false }: { initialDesign: DeckDesignV4; initialMessage?: string; startWithPhotos?: boolean }) {
  const [history, dispatch] = useReducer(designHistoryReducerV4, initialDesign, createHistoryV4);
  const [preview, setPreview] = useState<DeckDesignV4 | null>(null);
  ...
  const compatibilityDesign = useMemo(() => deckDesignV4ToV3Compatibility(design), [design]);
  const compatibilityPlatform = compatibilityDesign.platforms.find((item) => item.id === platform.id)!;
  const geometry = useMemo(() => derivePlatformGeometryV4(design, platform.id), [design, platform.id]);
  const houseGeometry = useMemo(() => deriveHouseContextGeometry(design.siteContext), [design.siteContext]);
  const projection = useMemo(() => deriveDeckDesignProjectionV4(design), [design]);
  ...
  const apply = (next: DeckDesignV4, nextMessage: string) => { setPreview(null); dispatch({ type: "apply", design: next }); setMessage(nextMessage); };
  ...
  const replaceRegion = (outer: readonly Point[], commit: boolean, holes = platform.region.holes): boolean => {
    try {
      const result = applyPolygonRegionReplacement(deckDesignV4ToV3Compatibility(history.present), platform.id, { outer, holes });
      const restored = restoreV4BeamLines(history.present, result.design);
      if (commit) apply(restored, result.notices.join(" ")); else setPreview(restored);
      return true;
    } catch (error) {
      setPreview(null);
  ...
  const updatePlatform = (update: Partial<DeckPlatformV4>, nextMessage: string) => {
    const current = history.present.platforms.find((item) => item.id === platform.id)!;
    apply(revisePlatform(history.present, { ...current, ...update }), nextMessage);
  };
  const updateConstruction = (construction: DeckPlatformV4["construction"], nextMessage: string) => updatePlatform({ construction }, nextMessage);
  const moveBeam = (requestedInset: number, previewOnly = false) => {
    try {
      ...
      const next = updateBeamLineV4(history.present, current.id, { ...beam, offsetFromOutside: inset }).design;
  ...
  const keepSelectedLevelOnly = () => {
    ...
    const next = normalizeDeckDesignV4({ ...history.present, platforms: [kept], metadata: { ...history.present.metadata, revision: history.present.metadata.revision + 1 } });
  ...
  const applyRailing = (railing: DeckPlatformV4["construction"]["railing"], nextMessage: string) => updateConstruction({ ...platform.construction, railing }, nextMessage);
  ...
  const updateHouseConnection = (next: DeckDesignV4, attachment: "unknown" | "ledger" | "non-ledger") => apply(next, attachment === "unknown" ? "House updated; connection needs field review." : "House connection updated.");
  const download = () => { const url = URL.createObjectURL(new Blob([stableDeckDesignV4Json(design)], { type: "application/json" })); const link = document.createElement("a"); link.href = url; link.download = "deck-design-v3.json"; link.click(); URL.revokeObjectURL(url); };
  const applyTemplate = (kind: "rectangle" | "l-shape") => {
    ...
    const next = migrateDeckDesignToV4({ ...legacy, id: design.id, name: design.name, metadata: { ...legacy.metadata, revision: design.metadata.revision + 1 } });
  ...
  const startFromPhotos = async (facts: ConfirmedPhotoFacts, review: PhotoIntakeReview, photoCount: number, confirmedOuter?: readonly PolygonPoint[], stairEdgeId?: string | null, stairOffset?: number | null, stairWidth?: number) => {
    ...
    const next = migrateDeckDesignToV4(createDesignFromConfirmedPhotoFacts(deckDesignV4ToV3Compatibility(history.present), facts, confirmedOuter, stairEdgeId, stairOffset, stairWidth));
    setPreview(null);
  ...
  return <main>
    ...
    <header className="topbar"><div className="brand-mark">M</div><div><p className="eyebrow">McKenzie Construction · isolated R&amp;D</p><h1>Deck Designer</h1></div><div className="header-actions"><button className="quiet" onClick={() => setPhotoIntakeOpen(true)}>Start with photos</button><button className="quiet" onClick={() => { saveDeckDesignV4(localStorage, design); setMessage(`Saved v4 locally at revision ${design.metadata.revision}.`); }}>Save locally</button><button className="quiet" onClick={download}>Download JSON</button><button className="primary" onClick={() => fileInput.current?.click()}>Load JSON</button><input ref={fileInput} hidden type="file" accept="application/json,.json" onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; try { const next = migrateDeckDesignToV4(JSON.parse(await file.text())); dispatch({ type: "reset", design: next }); setSelectedPlatformId(next.platforms[0].id); setWorkflowStage("layout"); setSelectedEdgeId(null); setSelectedStairSystemId(null); setSelectedLandingId(null); setSelectedHoleIndex(null); setMessage(`Loaded v4 design “${next.name}”.`); } catch (error) { setMessage(error instanceof Error ? `Load rejected: ${error.message}` : "Load rejected."); } event.target.value = ""; }} /></div></header>
    ...
    <div className="workspace"><aside className="controls-panel" id="design-controls">
      ...
      <label className="field full"><span>Design name</span><input value={design.name} onChange={(event) => { try { apply(normalizeDeckDesignV4({ ...history.present, name: event.target.value, metadata: { ...history.present.metadata, revision: history.present.metadata.revision + 1 } }), "Design name updated."); } catch { /* retain */ } }} /></label>
      ...
      <Suspense fallback={<div className="house-editor-loading" role="status">Preparing house connection…</div>}><HouseConnectionEditor design={compatibilityDesign} platform={compatibilityPlatform} onApply={(next, attachment) => updateHouseConnection(restoreV4BeamLines(history.present, next), attachment)} onError={setMessage} /></Suspense>
      ...
      <article className="view-card plan-card"><div className="card-title"><div><span>{workflowStage === "layout" ? "Measured plan" : "Railing plan"}</span><small>{workflowStage === "layout" ? `2D · ${platform.id} · ${geometry.platformEdges.length} sides · single level` : "2D · tap a railing side"}</small></div><div className="plan-card-tools"><strong>{formatFeetInches(platform.elevation)} above grade</strong><div className="plan-history-actions"><button disabled={!history.past.length} onClick={() => changeHistory("undo")}>Undo</button><button disabled={!history.future.length} onClick={() => changeHistory("redo")}>Redo</button></div></div></div><PlanViewV3 platform={compatibilityPlatform} activeStairSystem={activeStairSystem} geometry={visibleGeometry} houseGeometry={houseGeometry} snapIncrement={snapIncrement} editingEnabled={workflowStage === "layout"} selectedEdgeId={selectedEdgeId} selectedHoleIndex={selectedHoleIndex} beamLines={platform.construction.framing.beamLines} selectedBeamLineId={selectedBeamLineId} onSelectBeamLine={setSelectedBeamLineId} onSelectEdge={workflowStage === "layout" ? selectLayoutEdge : setSelectedEdgeId} onSelectStairSystem={(systemId) => selectStairObject(systemId)} onSelectLanding={(systemId, landingId) => selectStairObject(systemId, landingId)} onSelectHouseOpening={selectHouseOpening} onSelectHole={(index) => { setSelectedHoleIndex(index); setSelectedEdgeId(null); setSelectedStairSystemId(null); setSelectedLandingId(null); setMessage(`Cutout ${index + 1} selected. Drag the center to move it or a corner to resize it.`); }} onHolePreview={(index, hole) => moveCutout(index, hole, false)} onHoleCommit={(index, hole) => moveCutout(index, hole, true)} onCornerPreview={(index, point) => moveCorner(index, point, false)} onCornerCommit={(index, point, magnetic) => moveCorner(index, point, true, magnetic)} onCancel={() => setPreview(null)} onStairPreview={(offset) => moveActiveStairs(offset, "", true)} onStairCommit={(offset) => moveActiveStairs(offset, `Stairs moved to ${formatFeetInches(offset)}.`)} onSegmentPreview={(index, distance) => moveSegment(index, distance, false)} onSegmentCommit={(index, distance) => moveSegment(index, distance, true)} onBeamPreview={(inset) => moveBeam(inset, true)} onBeamCommit={(inset) => moveBeam(inset)} />
        ...
        </section> : <Suspense fallback={<div className="mobile-plan-edge-actions"><p>Preparing railing controls…</p></div>}><RailingMobileActions platform={compatibilityPlatform} geometry={geometry} selectedEdgeId={selectedEdgeId} onRailingChange={applyRailing} /></Suspense>}
      </article>
      <article className="view-card three-card"><div className="card-title"><div><span>{workflowStage === "layout" ? "Model view" : "Railing model"}</span><small>3D · single level</small></div><div className="view-tools"><select value={quality} aria-label="3D quality" onChange={(event) => setQuality(event.target.value as RenderQuality)}><option value="economy">Economy</option><option value="balanced">Balanced</option><option value="detailed">Detailed</option></select><div className="camera-buttons">{(["perspective", "top", "front"] as CameraPreset[]).map((value) => <button key={value} className={preset === value ? "active" : ""} onClick={() => { setPreset(value); setPresetRequest((current) => current + 1); }}>{value}</button>)}</div></div></div><Suspense fallback={<div className="three-loading">Preparing model…</div>}><ThreeViewV3 platform={compatibilityPlatform} geometry={visibleGeometry} contextPlatforms={[]} houseGeometry={houseGeometry} gradeElevation={design.siteContext.gradeElevation} preset={preset} presetRequest={presetRequest} showFraming={showFraming} quality={quality} /></Suspense><label className="check-row three-framing"><input type="checkbox" checked={showFraming} onChange={(event) => setShowFraming(event.target.checked)} />Show framing</label></article>
    </section></div>
    <section className="quantity-section"><div className="quantity-heading"><div><p className="eyebrow">Deterministic v4</p><h2>Conceptual quantities</h2></div><p>{photoStartSummary?.review.outlineWarning ? "Envelope quantities until reshaped." : "No prices or structural claims."}</p></div><div className="quantity-grid">{projection.aggregateQuantities.map((line) => <article className="quantity-card" key={line.key}><span>{line.key.replaceAll("-", " ")}</span><strong>{line.amount.toLocaleString()} <small>{line.unit}</small></strong><p>{line.sourceGeometry.length} geometry references.</p></article>)}</div></section>
  </main>;
}
