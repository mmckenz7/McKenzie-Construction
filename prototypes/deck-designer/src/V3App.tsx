...
export function V3App({ initialDesign, initialMessage = "Corner editor ready.", startWithPhotos = false }: { initialDesign: DeckDesignV3; initialMessage?: string; startWithPhotos?: boolean }) {
  ...
  return <main>
    ...
    <div className="workspace"><aside className="controls-panel" id="design-controls">
      ...
      <div className="field-grid"><V3NumberField label="Joist spacing (in)" value={platform.construction.framing.joistSpacing} onCommit={(value) => updateConstruction({ ...platform.construction, framing: { ...platform.construction.framing, joistSpacing: value } }, "Joist layout spacing updated.")} /><V3NumberField label="Beam from outside edge (feet)" value={Math.round(effectiveBeamInsetV3(platform) / 12 * 100) / 100} step={.5} onCommit={(value) => moveBeam(value * 12)} /></div>
