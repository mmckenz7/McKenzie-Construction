...
export function V3App({ initialDesign, initialMessage = "Corner editor ready." }: { initialDesign: DeckDesignV3; initialMessage?: string }) {
  ...
  return <main>
    ...
    <div className="workspace"><aside className="controls-panel">
      ...
      {hasEdgeReferences && <section className="selected-edge-card review-card"><strong>Outline references are protected</strong><p>Moving corners may change edge identities used by the house, railing, or stairs.</p><button onClick={unlockOutline}>Unlock outline editing</button><small>This explicit action clears house-edge intent and railing, and turns stairs off. Reattach them after shaping.</small></section>}
      <label className="field full"><span>Offset edge</span><select value={selectedEdgeId ?? ""} onChange={(event) => setSelectedEdgeId(event.target.value || null)}><option value="">Select an edge…</option>{geometry.platformEdges.map((edge, index) => <option value={edge.id} key={edge.id}>Edge {index + 1} · {formatFeetInches(edge.length)}</option>)}</select></label>
      <button className="primary full-action" onClick={addOffset}>Add rectangular offset</button><p className="section-help">Adds a 2-foot-deep rectangular offset to the selected edge. Click it again on another edge for a deck with two offsets, then drag corners or enter exact coordinates.</p>
