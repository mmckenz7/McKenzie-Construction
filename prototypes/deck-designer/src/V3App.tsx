...
import { clampBeamInsetV3, effectiveBeamInsetV3 } from "./framingEditorV3";
...
export function V3App({ initialDesign, initialMessage = "Corner editor ready.", startWithPhotos = false }: { initialDesign: DeckDesignV3; initialMessage?: string; startWithPhotos?: boolean }) {
  ...
  const moveBeam = (requestedInset: number, previewOnly = false) => {
    try {
      const current = history.present.platforms.find((item) => item.id === platform.id)!;
      const inset = clampBeamInsetV3(current, requestedInset);
      const next = revisePlatform(history.present, { ...current, construction: { ...current.construction, framing: { ...current.construction.framing, beamInset: inset } } });
  ...
  return <main>
    ...
    <div className="workspace"><aside className="controls-panel" id="design-controls">
      ...
      <div className="field-grid"><V3NumberField label="Joist spacing (in)" value={platform.construction.framing.joistSpacing} onCommit={(value) => updateConstruction({ ...platform.construction, framing: { ...platform.construction.framing, joistSpacing: value } }, "Joist layout spacing updated.")} /><V3NumberField label="Beam from outside edge (feet)" value={Math.round(effectiveBeamInsetV3(platform) / 12 * 100) / 100} step={.5} onCommit={(value) => moveBeam(value * 12)} /><V3NumberField label="Max conceptual post spacing (feet)" value={Math.round(platform.construction.framing.maxPostSpacing / 12 * 100) / 100} step={.5} onCommit={(value) => updateConstruction({ ...platform.construction, framing: { ...platform.construction.framing, maxPostSpacing: value * 12 } }, "Maximum conceptual post spacing updated; supports redistributed evenly.")} /></div>
