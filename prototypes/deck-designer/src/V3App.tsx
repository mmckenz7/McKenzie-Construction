    ...
    <div className="workspace"><aside className="controls-panel" id="design-controls">
      ...
      {activeStairSystem && !activeStairSystem.locked && <section className="selected-edge-card stair-configuration">
        ...
        <p>Position, width, landing shortcuts, and Done stay with the blueprint.</p>
          ...
          <button className="remove-stairs" onClick={() => { const remaining = platform.construction.stairSystems.filter((system) => system.id !== activeStairSystem.id); replaceStairSystems(remaining, "Stairs removed; they can be added again."); setSelectedStairSystemId(null); setSelectedLandingId(null); setSelectedEdgeId(null); }}>Remove this stair system</button>
      </section>}
      ...
      <article className="view-card plan-card"><div className="card-title"><div><span>{workflowStage === "layout" ? "Measured plan" : "Railing plan"}</span><small>{workflowStage === "layout" ? `2D · ${platform.id} · ${platform.region.outer.length} corners` : "2D · tap a railing side"}</small></div><strong>{formatFeetInches(platform.elevation)} high</strong></div><PlanViewV3 platform={platform} activeStairSystem={activeStairSystem} geometry={visibleGeometry} houseGeometry={houseGeometry} snapIncrement={snapIncrement} editingEnabled={workflowStage === "layout"} selectedEdgeId={selectedEdgeId} onSelectEdge={setSelectedEdgeId} onCornerPreview={(index, point) => moveCorner(index, point, false)} onCornerCommit={(index, point) => moveCorner(index, point, true)} onCancel={() => setPreview(null)} onStairPreview={(offset) => updateStairSystem({ offset }, "", true)} onStairCommit={(offset) => updateStairSystem({ offset }, `Stairs moved to ${formatFeetInches(offset)}.`)} onSegmentPreview={(index, distance) => moveSegment(index, distance, false)} onSegmentCommit={(index, distance) => moveSegment(index, distance, true)} />
        {workflowStage === "layout" ? <section className={`plan-action-tray${activeStairSystem ? " editing-stairs" : ""}`} aria-live="polite">
          {activeStairSystem ? activeStairSystem.locked ? <>
            <div className="plan-action-copy"><strong>Stair system {platform.construction.stairSystems.findIndex((system) => system.id === activeStairSystem.id) + 1} · done</strong><small>Reopen to make changes, or close to select another deck side.</small></div>
            <div className="plan-action-buttons"><button onClick={() => updateStairSystem({ locked: false }, "Stair system reopened.")}>Edit stairs</button><button className="primary" onClick={() => { setSelectedStairSystemId(null); setSelectedLandingId(null); }}>Close</button></div>
          </> : <>
            <div className="plan-action-copy"><strong>Stair system {platform.construction.stairSystems.findIndex((system) => system.id === activeStairSystem.id) + 1} · editing</strong><small>Drag for rough placement. Use these fields for exact placement.</small></div>
            <div className="plan-action-fields"><V3NumberField label="Position" value={activeStairSystem.offset} onCommit={(value) => updateStairSystem({ offset: value }, "Stair position updated exactly.")} /><V3NumberField label="Width" value={activeStairSystem.width} onCommit={(value) => updateStairSystem({ width: value, landings: activeStairSystem.landings.map((landing) => ({ ...landing, width: Math.max(landing.width, value) })) }, "Stair width updated exactly.")} /></div>
            <div className="plan-action-buttons"><button disabled={activeStairSystem.landings.some((landing) => !landing.locked || landing.afterRiser === 0)} onClick={() => addLanding("top")}>Top landing</button><button disabled={activeTotalRisers < 2 || activeStairSystem.landings.some((landing) => !landing.locked || landing.afterRiser === activeTotalRisers - 1)} onClick={() => addLanding("midway")}>Midway landing</button><button className="primary" onClick={lockStairSystem}>Done</button></div>
          </> : selectedEdgeId ? (() => { const edge = geometry.platformEdges.find((item) => item.id === selectedEdgeId)!; const isFree = platform.edgeConditions.some((condition) => condition.edgeId === edge.id && condition.condition === "free"); return <>
            <div className="plan-action-copy"><strong>{formatFeetInches(edge.length)} side selected</strong><small>{isFree ? "Choose what belongs on this side." : "House side selected. Stairs are unavailable here."}</small></div>
            <div className="plan-action-buttons"><button disabled={hasEdgeReferences} onClick={() => addBumpoutToEdge(edge.id)}>{hasEdgeReferences ? "Edit shape first" : "Add bumpout"}</button><button className="primary" disabled={!isFree || edge.length < 48} onClick={() => addStairsToEdge(edge.id, edge.length)}>Add stairs</button></div>
          </>; })() : <div className="plan-action-copy"><strong>Select a blueprint side</strong><small>Bumpout and stair actions will appear here.</small></div>}
