...
export function V3App({ initialDesign, initialMessage = "Corner editor ready.", startWithPhotos = false }: { initialDesign: DeckDesignV3; initialMessage?: string; startWithPhotos?: boolean }) {
  ...
  return <main>
    ...
    <div className="workspace"><aside className="controls-panel" id="design-controls">
      ...
      {platform.construction.stairs.enabled && <section className="selected-edge-card stair-configuration"><strong>Movable stairs</strong><p>Drag the orange-outlined handle in the plan. Add a landing only when the stairs need room before descending.</p><div className="field-grid"><V3NumberField label="Stair position" value={platform.construction.stairs.offset} onCommit={(value) => updateStairs({ offset: value }, "Stair position updated exactly.")} /><V3NumberField label="Stair width" value={platform.construction.stairs.width} onCommit={(value) => updateStairs({ width: value }, "Stair width updated exactly.")} /><V3NumberField label="Step depth" value={platform.construction.stairs.treadDepth} onCommit={(value) => updateStairs({ treadDepth: value }, "Step depth updated exactly.")} /></div><div className="stair-railing-note"><strong>Stair railings included</strong><small>Both descending sides are tracked separately from standard deck railings.</small></div><label className="check-row landing-toggle"><input type="checkbox" checked={platform.construction.stairs.landingEnabled} onChange={(event) => updateStairs({ landingEnabled: event.target.checked }, event.target.checked ? "Top landing added." : "Top landing removed; stairs return to a straight run.")} />Add top landing</label>{platform.construction.stairs.landingEnabled && <div className="stair-landing-controls"><V3NumberField label="Landing depth" value={platform.construction.stairs.landingDepth} onCommit={(value) => updateStairs({ landingDepth: value }, "Landing depth updated exactly.")} /><fieldset><legend>Direction after landing</legend><div className="toggle-grid">{(["straight", "left", "right"] as const).map((turn) => <button type="button" key={turn} className={`toggle${platform.construction.stairs.landingTurn === turn ? " active" : ""}`} onClick={() => updateStairs({ landingTurn: turn, landingDepth: turn === "straight" ? platform.construction.stairs.landingDepth : Math.max(platform.construction.stairs.landingDepth, platform.construction.stairs.width) }, `Stairs now continue ${turn} after the landing.`)}>{turn}</button>)}</div><small>Left and right are viewed while walking down from the deck.</small></fieldset><div className="landing-railing-note"><strong>Landing railings included</strong><small>Protected landing sides are added automatically; the stair opening stays open.</small></div></div>}<button className="remove-stairs" onClick={() => updateStairs({ enabled: false }, "Stairs removed.")}>Remove stairs</button></section>}
