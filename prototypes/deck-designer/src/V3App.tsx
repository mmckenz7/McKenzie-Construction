...
export function V3App({ initialDesign, initialMessage = "Corner editor ready.", startWithPhotos = false }: { initialDesign: DeckDesignV3; initialMessage?: string; startWithPhotos?: boolean }) {
  ...
  const enterRailingStage = () => { setPreview(null); setSelectedEdgeId(null); setSelectedStairSystemId(null); setSelectedLandingId(null); setWorkflowStage("railings"); setMessage("Layout locked. Tap railing sides."); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const returnToLayoutStage = () => { setSelectedEdgeId(null); setSelectedStairSystemId(null); setSelectedLandingId(null); setWorkflowStage("layout"); setMessage("Layout reopened; attachments stay protected."); window.scrollTo({ top: 0, behavior: "smooth" }); };
        ...
        {workflowStage === "layout" ? <section className={`plan-action-tray${activeStairSystem ? " editing-stairs" : ""}`} aria-live="polite">
          {selectedEdgeId ? (() => {
            ...
            return <>
              ...
              <div className="plan-action-fields segment-fields"><V3NumberField label="Side length (feet)" value={Math.round(edge.length / 12 * 100) / 100} step={.5} onCommit={(value) => updateSegmentLength(edge.id, value * 12)} />{activeStairSystem && !activeStairSystem.locked && <><V3NumberField label="Stair position (in)" value={activeStairSystem.offset} onCommit={(value) => updateStairSystem({ offset: value }, "Stair position updated exactly.")} /><V3NumberField label="Stair width (in)" value={activeStairSystem.width} onCommit={(value) => updateStairSystem({ width: value, landings: activeStairSystem.landings.map((landing) => ({ ...landing, width: Math.max(landing.width, value) })) }, "Stair width updated exactly.")} /><V3NumberField label="Step depth (in)" value={activeStairSystem.treadDepth} onCommit={(value) => updateStairSystem({ treadDepth: value }, "Step depth updated exactly.")} /></>}</div>
              {!activeStairSystem ? <div className="plan-action-buttons"><button disabled={hasEdgeReferences} onClick={() => addBumpoutToEdge(edge.id)}>{hasEdgeReferences ? "Edit shape first" : "Add bumpout"}</button><button className="primary" disabled={!isFree || edge.length < 48} onClick={() => addStairsToEdge(edge.id, edge.length)}>Add stairs</button></div> : activeStairSystem.locked ? <div className="plan-action-buttons"><button onClick={() => updateStairSystem({ locked: false }, "Stairs reopened for editing.")}>Edit stairs</button><button className="primary" onClick={() => { setSelectedEdgeId(null); setSelectedStairSystemId(null); setSelectedLandingId(null); }}>Close side</button></div> : <>
                <div className="plan-action-buttons"><button disabled={activeStairSystem.landings.some((landing) => !landing.locked || landing.afterRiser === 0)} onClick={() => addLanding("top")}>Add top landing</button><button disabled={activeTotalRisers < 2 || activeStairSystem.landings.some((landing) => !landing.locked || landing.afterRiser === activeTotalRisers - 1)} onClick={() => addLanding("midway")}>Add midway landing</button><button className="primary" onClick={lockStairSystem}>Done with side</button></div>
                ...
                {activeLanding && <div className="stair-landing-controls segment-landing-controls">
                  ...
                  <div className="field-grid"><V3NumberField label="Landing width (in)" value={activeLanding.width} onCommit={(value) => updateLanding({ width: value }, "Landing width updated exactly.")} /><V3NumberField label="Landing depth (in)" value={activeLanding.depth} onCommit={(value) => updateLanding({ depth: value }, "Landing depth updated exactly.")} /></div>
