    ...
    <section className="workspace">
      <div className="canvas-shell">
        ...
        <svg ref={svgRef} className={`plan-canvas ${mode}${isNavigating ? " navigating" : ""}`} viewBox={`${view.x} ${view.y} ${view.width} ${view.height}`} onPointerDown={startNavigation} onPointerMove={moveCanvasPointer} onPointerLeave={() => { if (!drag && !isNavigating) setPreviewPoint(null); }} onPointerUp={endNavigation} onPointerCancel={endNavigation} aria-label="Fence drawing plan">
          ...
          {selectedSegment && (() => {
            const reference = selectedSegment.kind === "gate" ? selectedGateRun : gateEditorOpen ? { postA: pointById(design, selectedSegment.fromPointId), postB: pointById(design, selectedSegment.toPointId) } : null;
            if (!reference) return null;
            return <g className="gate-reference-posts top-layer" pointerEvents="none" aria-label={`Editable gate reference posts. ${gateReferencePost === "post-a" ? "Post A" : "Post B"} selected`}>
              <g className={gateReferencePost === "post-a" ? "active" : ""} transform={`translate(${reference.postA.xMm} ${reference.postA.yMm})`}><circle className="halo" r={470 * dimensionScale} /><circle className="badge" r={345 * dimensionScale} /><text textAnchor="middle" dominantBaseline="central" style={{ fontSize: 310 * dimensionScale }}>A</text></g>
      ...
      <aside className={`inspector${selection ? " has-selection" : ""}`}>
        ...
        {selectedSegment && <div>
          ...
          {selectedSegment.kind === "gate" && selectedGateRun && <><div className="gate-position-readout" aria-label="Choose gate measurement post"><button type="button" aria-pressed={gateReferencePost === "post-a"} className={gateReferencePost === "post-a" ? "active" : ""} onClick={() => { setGateReferencePost("post-a"); setNotice("Post A selected as the gate measurement origin."); }}><span>Post A to nearest gate edge</span><strong>{formatFeetInches(selectedGateRun.offsetFromPostAMm)}</strong></button><button type="button" aria-pressed={gateReferencePost === "post-b"} className={gateReferencePost === "post-b" ? "active" : ""} onClick={() => { setGateReferencePost("post-b"); setNotice("Post B selected as the gate measurement origin."); }}><span>Post B to nearest gate edge</span><strong>{formatFeetInches(selectedGateRun.runLengthMm - selectedGateRun.offsetFromPostAMm - segmentLengthMm(design, selectedSegment))}</strong></button></div><button className="wide" onClick={() => commit(setSegmentKind(design, selectedSegment.id, "fence"), "Gate opening restored to fence intent.")}>Mark as fence</button></>}
