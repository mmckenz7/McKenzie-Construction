...
The neutral contract is accepted and the no-billing Demo Key is configured for the restricted Fence Preview. The renderer spike tests Google satellite/hybrid + local parcel GeoJSON + read-only Fence + observational browser GPS in one coordinate view, with no geocoding and no provider persistence. KGIS-approved access evidence remains separate. Do not resume consumer-phone Site Walk as authoritative residential measurement.
