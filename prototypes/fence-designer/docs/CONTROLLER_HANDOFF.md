...
Created a usable, isolated 2D fence measurement prototype under `prototypes/fence-designer/` with one synchronized graphical/exact-command layout workflow and selectable no-pricing **Black Aluminum** and **Treated Pine Privacy** takeoffs. The bounded Google candidate adds a disposable Maps JavaScript satellite/hybrid renderer, local sanitized parcel GeoJSON/KML overlay, browser accuracy-circle observation, and local GeoJSON/KML export. Canonical integer-mm Fence geometry, gates, history, and takeoff remain unchanged and provider-free; provider failure preserves the local editor. No geocoding, Places, Street View, provider persistence, KGIS fetch, live third-party sync, key value, paid billing resource, or Production activation is present.

The owner authorized a no-billing Google Maps Demo Key for this non-Production candidate. It is stored only as the hidden `GOOGLE_MAPS_BROWSER_KEY` value on the `codex/fence-visual-measure` Preview, restricted to the exact stable Fence Preview origin and Maps JavaScript API. Google's documented daily Demo Key limit pauses service rather than creating charges. Paid billing, other branch scopes, and Production remain excluded.
...
- `src/GoogleMapSpike.tsx`: restricted-Preview candidate panel and the only integration surface between the adapter and existing Fence editor commands.
...
- 159 deterministic tests passed, including authored first/middle/last-span edits, multiple downstream points, two downstream gates, separate-line isolation, reverse bearings, invalid and connected topology, no-op revisions, deterministic replay/save-load, portable-job round trips and rejection paths, separate crash recovery, independent-line scale residuals, saved-scale re-verification, ground-plane registration, generic immutable point/polyline/polygon presentation, neutral/Fence interaction separation, provider mount/destroy/offline behavior, local GeoJSON/KML parsing/export, observational-location stops, and provenance non-promotion.
...
- The provider-neutral contract is a tested prototype boundary, not a provider selection or persisted shared-domain implementation. Google remains a candidate. No provider package, key value, billing account, map ID, geocoder, persisted provider object, or Production resource was added. The restricted Demo Key is injected only into the approved Fence Preview. Formal provenance attachment to accepted persistent geometry still requires Controller alignment with the shared Fence domain contract.
...
- The portable Fence job file moves canonical geometry and gate intent between devices, but intentionally excludes customer data, imagery, provider state, pricing, and takeoff confirmation. Same-browser crash recovery is best effort and starts a fresh undo history after refresh.
- The printable packet is a browser print/PDF view of the currently confirmed preliminary takeoff. It is not a quote, contract, purchase order, or permanent OS record.
...
- Google Earth imagery cannot be uploaded into this commercial web tool. The Maps JavaScript candidate uses only Google's no-billing Demo Key on the restricted non-Production Fence Preview.

## Owner decisions needed later

The no-billing Google Maps Demo Key activation has been authorized and configured for exactly one non-Production Fence Preview origin and Maps JavaScript API. The Demo Key's documented daily limit pauses service rather than permitting paid overage. No Geocoding or Places is needed. A paid Maps Platform key, billing account, broader Preview scope, or Production use requires a new exact approval. Sources: [Demo Key](https://developers.google.com/maps/documentation/javascript/demo-key) and [API-key security](https://developers.google.com/maps/api-security-best-practices).
...
The active candidate uses a no-billing Demo Key instead of the earlier paid-key guardrail proposal. Its limits are Google's Demo Key daily pauses; there is no paid spend cap because no billing account is attached. If the Demo Key proves insufficient, stop and obtain a new exact owner decision before creating a paid key, attaching billing, or enabling additional APIs.
...
Run the single integrated production-candidate acceptance pass in `docs/BROWSER_QA.md`: property reference, rough layout, authored exact review, two gate types, portable export/import, crash recovery, both preliminary takeoffs, print/PDF, and 390 px mobile controls. Hold CRM persistence, pricing, product-catalog selection, and customer/job linkage until that pass identifies any workflow defects and the relevant shared-domain reviews are complete. KGIS-approved access evidence remains separate. Do not resume consumer-phone Site Walk as authoritative residential measurement.
