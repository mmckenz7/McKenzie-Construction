...
- 137 deterministic tests passed, including authored first/middle/last-span edits, multiple downstream points, two downstream gates, separate-line isolation, reverse bearings, invalid and connected topology, no-op revisions, deterministic replay/save-load, independent-line scale residuals, saved-scale re-verification, ground-plane registration, layer/geometry isolation, and provenance non-promotion.
...
The newly requested Google-primary evaluation fits the same neutral contract and should be the first candidate tested once authorized. The smallest spike needs only Maps JavaScript API on a separate non-Production website-restricted key, limited to the approved Preview origins and restricted at the API level to Maps JavaScript API. It does **not** need Geocoding or Places: use one approved fixed test coordinate. If address lookup is later added to the evaluation, authorize Geocoding separately rather than broadening the renderer key silently. Google requires billing and authenticated API requests; its current Dynamic Maps allowance is 10,000 no-cost monthly events and then $7 per 1,000 in the first paid tier. The existing 2,000-load ceiling therefore remains expected at $0 but still needs a named billing owner, quotas/alerts, and an enforced-stop mechanism. Google recommends both website and API restrictions and separate keys per application. Sources: [usage and billing](https://developers.google.com/maps/documentation/javascript/usage-and-billing), [current pricing](https://developers.google.com/maps/billing-and-pricing/pricing), and [API-key security](https://developers.google.com/maps/api-security-best-practices).
