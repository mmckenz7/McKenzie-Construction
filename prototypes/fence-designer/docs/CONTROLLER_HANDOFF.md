...
Created a usable, isolated 2D fence measurement prototype under `prototypes/fence-designer/` with selectable no-pricing **Black Aluminum** and **Treated Pine Privacy** takeoffs. All materials use one authoritative material-agnostic drawing workflow. Material controls appear only after explicit confirmation of the current design revision; any later geometry change invalidates that confirmation without altering the drawing. Zoom-stable point markers preserve the drawing tool's visible target size after fitting or zooming. The treated-pine calculator implements owner-supplied 6-foot height, touching 6-inch pickets, 8-foot maximum bays, three rails per bay, treated 4×4 posts, one 50 lb concrete bag per installed hole, 10% waste on all purchased lumber, six screws per installed picket, five 2×4 frame pieces per gate leaf, and two hinges per leaf. Single gates use five frame pieces; doubles use ten, four hinges, one latch, and one drop rod. Counts and copied reports remain pure derived session state with no products, prices, or schema extension. The existing Black Aluminum calculation and cut/post audit remain unchanged. **Property** also adds deterministic manual house straightening before fence drawing. Fence measurements are never rotated or promoted from imagery; automatic building detection is deferred.
...
The neutral contract is accepted. The later owner request is queued—not active—for one restricted non-Production browser key; the named $10 cap, rate quotas, usage/billing alerts, and billing owner above; and an approved deidentified Knoxville evaluation set for a time-boxed three-candidate renderer spike. In parallel, compare both material takeoffs against known completed layouts. The next treated-pine input is actual post stock length; the new structural-fastener defaults should be compared against one completed job before being treated as final. Focused Site Walk is browser-validated and now needs one short HTTPS phone field test: first GPS reference, two or three marked points, one exact-length correction, one separate-line start, local save/load, and a screenshot plus phone model/browser/accuracy observations.
