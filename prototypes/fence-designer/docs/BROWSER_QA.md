...
108. Opened **Property → Open live map beta** without a runtime key and confirmed the panel fails closed with a clear “Restricted Preview key not configured” status; no Google request was made and the local plan remained usable.
109. At a 390 × 844 viewport, confirmed the inactive map panel remained horizontally contained, its parcel/location/export controls were at least 48 pixels high, and GeoJSON/KML export remained disabled before deliberate map placement.
110. Confirmed the inactive panel exposes no address/geocoder, Places, Street View, live KGIS, or third-party synchronization action and reports browser location as observational only. Browser error/warning log was empty.
111. The owner later selected a no-billing Demo Key instead of a paid-key cost boundary. The key is restricted to the exact Fence Preview origin and Maps JavaScript API; live satellite/hybrid, attribution, gesture, overlay, and lifecycle evidence is recorded only after the new automatic Preview receives that branch-scoped value.
112. After extracting the domain-neutral read-only map presentation/registration core and moving Fence projection/draft translation into Fence-owned wrappers, reopened **Property → Open live map beta** and confirmed the existing inactive-key status and Fence workspace remained intact with no browser warnings/errors. At 390 × 844 the panel remained horizontally contained and its actions remained 48 pixels high.
