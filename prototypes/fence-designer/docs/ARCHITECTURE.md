...
Live length presentation is screen-space UI state: while a prospective run follows the pointer, its feet/inches value appears in a fixed canvas card rather than over the geometry. Completed dimensions are derived from authoritative segment length, offset perpendicular to their run, and connected by a visual leader. A deterministic placement pass tests each label's bounding box, tries both sides of its run, and increases the offset until nearby labels no longer collide. A session-only manual side override forces a selected label to its opposite side while still allowing greater offset on that side. Label size and offset scale with the plan view so zooming does not turn a short-run label into an oversized obstruction. Label placement never affects geometry, totals, history, or serialization.
