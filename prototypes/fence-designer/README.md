...
While drawing, the changing run length stays in a fixed on-canvas measurement box so it does not cover the target or reference image. After placement, the completed dimension is rendered beside its fence run with a leader and remains a stable visual size while zooming.
