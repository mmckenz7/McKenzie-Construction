...
export function CommunicationReplyComposer({
  ...
  return <form onSubmit={submitReply} className="space-y-4">
    ...
    <label className="block">
      ...
      <textarea className={`min-h-36 w-full resize-y rounded-lg border px-3 py-3 text-sm leading-6 outline-none focus:border-blue-500 ${inputClass}`} value={body} onChange={(event) => setBody(event.target.value)} maxLength={20000} placeholder="Write your reply…" required disabled={!recipient || submitting} />
    </label>
    ...
    <div className="flex flex-wrap items-center gap-3">
      <button type="submit" disabled={!recipient || submitting} className="rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-bold text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50">{submitting ? "Sending…" : "Send reply"}</button>
