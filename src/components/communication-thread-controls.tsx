...
export function CommunicationThreadControls({
  ...
  const buttonClass = compact
    ? "rounded-lg border border-slate-700 px-2.5 py-2 text-xs font-bold text-slate-300 hover:border-blue-700 hover:text-blue-300 disabled:opacity-50"
    : "rounded-lg border border-slate-700 px-3 py-2 text-xs font-bold text-slate-300 hover:border-blue-700 hover:text-blue-300 disabled:opacity-50";
  ...
  return <div className={compact ? "flex flex-wrap items-center justify-end gap-2" : "space-y-3"}>
    ...
    <button type="button" className={`${buttonClass} hover:border-amber-700 hover:text-amber-300`} disabled={busy} onClick={() => update({ status: status === "archived" ? "open" : "archived" })}>
      {status === "archived" ? "Restore" : "Archive"}
    </button>
    {!compact ? <label className="block text-xs font-bold uppercase tracking-wider text-slate-500">
      Assigned to
      <select
        className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm font-semibold normal-case tracking-normal text-slate-200"
        value={assignedToId ?? ""}
        disabled={busy}
