...
export function EstimateProposalCard({ estimateId, estimateStatus }: { estimateId: string; estimateStatus: string }) {
  ...
  async function revoke() {
    const expiredProposal = proposal?.status === "expired"
      || Boolean(proposal && new Date(proposal.expiresAt).getTime() <= Date.now());
    if (!window.confirm(expiredProposal
      ? "Open this expired estimate for pricing review? The old customer link will remain unavailable."
      : "Revoke this customer link and reopen the estimate for editing?")) return;
    setPending(true); setError(""); setNotice("");
    try {
  ...
  const canReopen = Boolean(active || proposal?.status === "expired");
  ...
  return <section className="rounded-2xl border border-blue-200 bg-blue-50 p-6 shadow-sm">
    ...
    {loading ? <p className="mt-5 text-sm font-semibold text-slate-600">Checking customer link…</p> : proposal && proposal.status !== "revoked" ? <div className="mt-5 rounded-xl border border-blue-200 bg-white p-4">
      ...
      {proposal.publicUrl && !expired ? <input readOnly aria-label="Customer estimate link" value={proposal.publicUrl} className="mt-4 w-full rounded-lg border border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-700" /> : null}
      <div className="mt-4 flex flex-wrap gap-3">
        {proposal.publicUrl && !expired ? <button type="button" className={primary} onClick={() => void copyLink()}>Copy link</button> : null}
        ...
        {canReopen ? <button type="button" disabled={pending} className={expired ? primary : secondary} onClick={() => void revoke()}>{pending ? "Working…" : expired ? "Review pricing and edit" : "Revoke and edit"}</button> : null}
      </div>
