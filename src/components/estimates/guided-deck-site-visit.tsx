...
type MeasurementDraft = { value: string; unit: string; feet?: string; inches?: string; components?: string[] };
...
const FIELD_GUIDANCE: Record<string, string> = {
  length: "Measure the longest deck edge from end to end.", width: "Measure from the house-side edge to the outside deck edge.", height_from_grade: "Measure vertically from visible grade to the top of the deck surface.",
  ledger_length: "Measure the visible house connection from end to end.", joist_spacing: "Measure horizontally from the center of one visible joist to the center of the next.", joist_depth: "Measure the vertical face of one visible joist.", beam_depth: "Measure the vertical face of one visible beam.",
  post_dimensions: "Measure the visible post face in both directions: width and depth.", support_spacing: "Measure the horizontal distance between adjacent support or beam lines. Do not assume this is post spacing.", exposed_footing_dimensions: "Measure only what is exposed: visible width, visible length, and visible height or depth. Do not estimate anything below grade.",
  stair_width: "Measure across the usable stair width.", total_rise: "Measure vertically from the bottom landing level to the top deck surface.", tread_depth: "Measure front to back on one representative tread.", representative_riser: "Measure vertically between two adjacent representative tread surfaces.", landing_dimensions: "Measure the visible landing and record the requested overall dimension without assuming concealed edges.",
  guard_height: "Measure vertically from the deck surface to the top of the guard.", opening: "Measure one representative clear opening between visible guard components.", rail_lengths_by_area: "Measure each visible railing run and enter the combined field-observed length for this capture.", handrail_height: "Measure vertically from the stair nosing line to the top of the handrail.",
  narrow_access_width: "Measure the narrowest clear width along the visible access route.", gate_width: "Measure the clear opening between the gate stops or posts.", clearance: "Measure the lowest visible overhead clearance on the access route.", obstruction_clearances: "Measure the shortest relevant visible clearance between the work area and the obstruction.",
};
const LONG_UNITS = ["ft", "ft + in", "in"] as const;
const DETAIL_UNITS = ["in", "ft", "ft + in"] as const;
const COMPOSITE_UNITS = ["in", "ft"] as const;
const COMPOSITE_FIELDS: Record<string, string[]> = { post_dimensions: ["Post width", "Post depth"], exposed_footing_dimensions: ["Visible footing width", "Visible footing length", "Visible footing height or depth"] };
...
const REVIEW_GUIDANCE: Record<string, { reason: string; action: string }> = {
  blurry: { reason: "The photo looks blurry.", action: "Hold the camera steady, tap the subject to focus, and take it again." }, too_dark: { reason: "The photo looks too dark.", action: "Add light or move to a brighter angle, then take it again." },
  too_bright: { reason: "The photo looks too bright.", action: "Move out of direct light or lower the camera angle, then take it again." }, glare: { reason: "Glare is hiding part of the view.", action: "Change your angle so light is not reflecting into the camera." },
  obstructed: { reason: "Something is blocking the view.", action: "Move around the obstruction when safe and show the requested area clearly." }, wrong_subject: { reason: "The requested area may not be in the photo.", action: "Check the requested-photo list and photograph that area." },
  incomplete_view: { reason: "The full requested area is not visible.", action: "Step back or change angle until every requested part is in frame." }, too_distant: { reason: "The important details are too far away.", action: "Move closer while keeping the full requested area in frame." },
  orientation_problem: { reason: "The photo orientation makes the view hard to check.", action: "Rotate the phone and retake the photo upright." }, unsupported_media: { reason: "This photo format could not be reviewed.", action: "Take a new photo or choose a JPEG, PNG, or WebP image." },
  review_unavailable: { reason: "The review provider did not return a usable result, so the exact photo problem is unknown.", action: "Retry the review. If it still fails, retake a clear photo or inspect this photo yourself." },
};
...
export function GuidedDeckSiteVisit({ estimateId }: { estimateId: string }) {
  ...
  const requirementSatisfied = reviewPhotoReady && humanAccepted
    ...
    && (!requiresFields || fieldNames.every((field) => measurementComplete(field, measurements[field])));
  ...
  async function reviewPhoto(photoId: string, idempotencyKey: string) {
    ...
    try {
      ...
      setLocalReview({ photoId, status: "complete", verdict: "unable_to_assess", issueCodes: ["review_unavailable"] });
    }
  }
  ...
  async function confirmItem() {
    ...
    try {
      ...
      const storedMeasurements = serializeMeasurements(fieldNames, measurements);
      const observation = current.requirement.mode === "photo_only" ? {}
        : current.requirement.mode === "required_measurements" ? { measurements: storedMeasurements }
        : condition === "yes" ? { conditionStatus: "applies", measurements: storedMeasurements }
...
function PhotoReviewStatus({ review, busy, onAccept, onRetry, onRetake, uploadPhoto }: { review: { status: "reviewing" } | { status: "complete"; verdict: UsabilityVerdict; issueCodes: string[] } | null; busy: boolean; onAccept: () => void; onRetry: () => void; onRetake: () => void; uploadPhoto: (file: File) => Promise<void> }) {
  ...
  if (review.verdict === "retake_recommended") return <div role="status" className="mt-4 rounded-lg border border-amber-400 bg-amber-50 p-4 text-amber-950"><p className="text-lg font-bold">Retake recommended</p><p className="mt-1 text-sm"><strong>{guidance?.reason ?? "The requested area is not clear enough to check."}</strong> {guidance?.action ?? "Take another photo that clearly shows every requested item."}</p><PhotoSourceControls title="Retake photo" busy={busy} uploadPhoto={uploadPhoto} onSelect={onRetake} cameraPrimary /><button type="button" disabled={busy} onClick={onAccept} className={`mt-3 ${secondary}`}>Use anyway — I checked it</button></div>;
  const unable = guidance ?? REVIEW_GUIDANCE.review_unavailable;
  return <div role="status" className="mt-4 rounded-lg border border-slate-300 bg-slate-50 p-4 text-slate-950"><p className="text-lg font-bold">Couldn’t review</p><p className="mt-1 text-sm"><strong>{unable.reason}</strong> {unable.action}</p><button type="button" disabled={busy} onClick={onRetry} className={`mt-4 ${primary}`}>Retry review</button><PhotoSourceControls title="Retake photo" busy={busy} uploadPhoto={uploadPhoto} onSelect={onRetake} /><button type="button" disabled={busy} onClick={onAccept} className={`mt-3 ${secondary}`}>Review photo myself</button></div>;
}
...
function ManualConfirmation({ current, condition, setCondition, measurements, setMeasurements }: { current: VisitItem; condition: string; setCondition: (value: "yes" | "no" | "") => void; measurements: Record<string, MeasurementDraft>; setMeasurements: (value: Record<string, MeasurementDraft>) => void }) {
  ...
  return <div className="mt-4 rounded-lg border border-slate-300 p-4"><h3 className="font-bold">Check photo and field facts</h3>{conditional ? <fieldset className="mt-3"><legend className="text-sm font-bold">{CONDITIONS[current.requirement.when ?? ""]?.prompt ?? "Does this condition apply?"}</legend><div className="mt-2 grid gap-2 sm:grid-cols-2">{(["yes", "no"] as const).map((value) => <label key={value} className="flex items-center gap-2 rounded-lg border border-slate-300 p-3 text-sm font-semibold"><input type="radio" name={`condition-${current.id}`} checked={condition === value} onChange={() => setCondition(value)} />{value === "yes" ? CONDITIONS[current.requirement.when ?? ""]?.yes ?? "Yes" : CONDITIONS[current.requirement.when ?? ""]?.no ?? "No"}</label>)}</div></fieldset> : null}{showFields ? <div className="mt-4 grid gap-4">{(current.requirement.fields ?? []).map((field) => <MeasurementField key={field} field={field} draft={measurements[field]} onChange={(draft) => setMeasurements({ ...measurements, [field]: draft })} />)}</div> : null}</div>;
}

function MeasurementField({ field, draft, onChange }: { field: string; draft?: MeasurementDraft; onChange: (draft: MeasurementDraft) => void }) {
  const parts = COMPOSITE_FIELDS[field];
  if (parts) return <fieldset><legend className="text-sm font-bold">{FIELD_LABELS[field]}</legend><p className="mt-1 text-xs leading-5 text-slate-600">{FIELD_GUIDANCE[field]}</p><div className="mt-2 grid gap-2 sm:grid-cols-3">{parts.map((part, index) => <label key={part} className="text-xs font-bold">{part}<input aria-label={part} type="number" min="0" step="any" inputMode="decimal" className={input} value={draft?.components?.[index] ?? ""} onChange={(event) => { const components = [...(draft?.components ?? [])]; components[index] = event.target.value; onChange({ value: components.join(" × "), unit: draft?.unit ?? "", components }); }} placeholder="Number" /></label>)}</div><UnitSelect field={field} value={draft?.unit ?? ""} onChange={(unit) => onChange({ ...draft, value: draft?.value ?? "", unit, components: draft?.components ?? [] })} units={COMPOSITE_UNITS} /></fieldset>;
  const mixed = draft?.unit === "ft + in";
  return <fieldset><legend className="text-sm font-bold">{FIELD_LABELS[field] ?? field}</legend><p className="mt-1 text-xs leading-5 text-slate-600">{FIELD_GUIDANCE[field]}</p>{mixed ? <div className="grid grid-cols-2 gap-2"><input aria-label={`${FIELD_LABELS[field]} feet`} type="number" min="0" step="any" inputMode="decimal" className={input} value={draft?.feet ?? ""} onChange={(event) => onChange({ ...draft, feet: event.target.value, value: `${event.target.value} ft ${draft?.inches ?? ""} in`, unit: "ft + in" })} placeholder="Feet" /><input aria-label={`${FIELD_LABELS[field]} inches`} type="number" min="0" step="any" inputMode="decimal" className={input} value={draft?.inches ?? ""} onChange={(event) => onChange({ ...draft, inches: event.target.value, value: `${draft?.feet ?? ""} ft ${event.target.value} in`, unit: "ft + in" })} placeholder="Inches" /></div> : <input aria-label={`${FIELD_LABELS[field] ?? field} value`} type="number" min="0" step="any" inputMode="decimal" className={input} value={draft?.value ?? ""} onChange={(event) => onChange({ value: event.target.value, unit: draft?.unit ?? "" })} placeholder="Number" />}<UnitSelect field={field} value={draft?.unit ?? ""} onChange={(unit) => onChange({ value: unit === "ft + in" ? "" : draft?.value ?? "", unit, feet: "", inches: "" })} units={longField(field) ? LONG_UNITS : DETAIL_UNITS} /></fieldset>;
}

function UnitSelect({ field, value, onChange, units }: { field: string; value: string; onChange: (unit: string) => void; units: readonly string[] }) { return <label className="mt-2 block text-xs font-bold">Unit<select aria-label={`${FIELD_LABELS[field] ?? field} unit`} className={input} value={value} onChange={(event) => onChange(event.target.value)}><option value="">Choose unit</option>{units.map((unit) => <option key={unit} value={unit}>{unit}</option>)}</select></label>; }
...
function longField(field: string) { return ["length", "width", "height_from_grade", "ledger_length", "support_spacing", "stair_width", "total_rise", "landing_dimensions", "rail_lengths_by_area", "narrow_access_width", "gate_width", "clearance", "obstruction_clearances"].includes(field); }
function measurementComplete(field: string, draft?: MeasurementDraft) { if (!draft?.unit) return false; if (COMPOSITE_FIELDS[field]) return COMPOSITE_FIELDS[field].every((_, index) => draft.components?.[index]?.trim()); return draft.unit === "ft + in" ? Boolean(draft.feet?.trim() && draft.inches?.trim()) : Boolean(draft.value.trim()); }
function serializeMeasurements(fields: string[], drafts: Record<string, MeasurementDraft>) { return Object.fromEntries(fields.map((field) => { const draft = drafts[field]; const value = COMPOSITE_FIELDS[field] ? draft.components!.map((part) => part.trim()).join(" × ") : draft.unit === "ft + in" ? `${draft.feet!.trim()} ft ${draft.inches!.trim()} in` : draft.value.trim(); return [field, { value, unit: draft.unit }]; })); }
