...
function PhotoReviewStatus({ review, busy, onAccept, onRetry, onRetake, uploadPhoto }: { review: { status: "reviewing" } | { status: "complete"; verdict: UsabilityVerdict; issueCodes: string[] } | null; busy: boolean; onAccept: () => void; onRetry: () => void; onRetake: () => void; uploadPhoto: (file: File) => Promise<void> }) {
  ...
  if (review.verdict === "usable") return <div role="status" className="mt-4 rounded-lg border border-emerald-300 bg-emerald-50 p-4 text-emerald-950"><p className="text-lg font-bold">Good</p><p className="mt-1 text-sm">The requested area appears clear enough to inspect. You still need to verify it.</p><div className="mt-4 flex flex-col gap-3 sm:flex-row"><button type="button" disabled={busy} onClick={onAccept} className={primary}>Use this photo</button><PhotoInput label="Retake photo" busy={busy} uploadPhoto={uploadPhoto} onChange={onRetake} /></div></div>;
  ...
  return <div role="status" className="mt-4 rounded-lg border border-slate-300 bg-slate-50 p-4 text-slate-950"><p className="text-lg font-bold">Couldn’t review</p><p className="mt-1 text-sm">{REVIEW_REASONS[review.issueCodes[0]] ?? "The automatic check did not finish. You can retry, retake the photo, or inspect it yourself."}</p><div className="mt-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><button type="button" disabled={busy} onClick={onRetry} className={primary}>Retry review</button><PhotoInput label="Retake photo" busy={busy} uploadPhoto={uploadPhoto} onChange={onRetake} /><button type="button" disabled={busy} onClick={onAccept} className={secondary}>Review photo myself</button></div></div>;
}
