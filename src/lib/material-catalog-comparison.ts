...
function convertQuantity(
  ...
  const path = candidate.conversionPath;
  if (path.length === 0) {
    if (request.requestedUnitId !== candidate.price.priceUnitId) {
      return { reason: "invalid_conversion_path" as const };
    }
    return { quantity: parseDecimal(request.requestedQuantity, "requestedQuantity") };
  }

  let currentUnitId = request.requestedUnitId;
  let quantity = parseDecimal(request.requestedQuantity, "requestedQuantity");
  for (const conversion of path) {
    if (
      conversion.productId !== request.productId ||
  ...
  if (currentUnitId !== candidate.price.priceUnitId) {
    return { reason: "invalid_conversion_path" as const };
  }
  return { quantity };
}
...
export function compareSupplierOffers(
  ...
  for (const candidate of request.candidates) {
    try {
      ...
      const converted = convertQuantity(request, candidate, pricedForMilliseconds);
      if ("reason" in converted) {
        exclusions.push(exclude(candidate, converted.reason));
        continue;
      }
      const preRoundPurchaseQuantity = converted.quantity;
      ...
      if (
        (tierMinimum !== null && compare(purchaseQuantity, tierMinimum) < 0) ||
        (tierMaximum !== null && compare(purchaseQuantity, tierMaximum) > 0)
      ) {
      ...
      const merchandiseCost = multiply(
        divide(purchaseQuantity, parseDecimal(candidate.price.priceQuantity, "priceQuantity")),
      ...
      comparisons.push({
        ...
        purchaseUnitId: candidate.price.priceUnitId,
        ...
        conversionPath: Object.freeze([...candidate.conversionPath]),
        rankValue,
      });
    } catch {
  ...
  comparisons.sort((left, right) => {
    ...
    return [left.supplierId, left.supplierLocationId ?? "", left.supplierSku, left.offerId, left.observationPriceId]
      .join("\u0000")
      .localeCompare(
  ...
  exclusions.sort((left, right) =>
    `${left.candidateId}\u0000${left.reason}`.localeCompare(`${right.candidateId}\u0000${right.reason}`),
  );
