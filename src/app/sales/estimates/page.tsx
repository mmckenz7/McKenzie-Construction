...
export default async function EstimatesPage() {
  ...
  const [{ data, error }, { data: drafts, error: draftError }] = await Promise.all([
    ...
    supabase.from("estimates").select("id, lead_id, status, updated_at")
      .in("status", ["draft", "reviewing", "sent", "viewed"])
      .eq("calculation_policy_version", "structured-estimate-v1")
      .order("updated_at", { ascending: false }),
  ]);
  ...
  for (const estimate of drafts ?? []) {
    ...
    if (!estimateByLead.has(leadId)) {
      estimateByLead.set(leadId, { id: String(estimate.id), status: String(estimate.status) });
    }
  }
